...
| 阿里山林鐵 | 本線／祝山／神木／沼平線定期與推算班次 | `data/afr_schedule_dense.json`、TDX＋OSM 缺口補線 `data/afr.json` |
...
| `build_afr.mjs` ／ `fetch_afr_osm_fills.mjs` | 阿里山林鐵 TDX 路網／班表，並以 OSM active narrow-gauge 軌道補上 TDX 的 5 處線形缺口 |
| `repair_station_distances.mjs` | 重投影已發布資料中漏算接縫的站點里程（目前為淡海藍海線末三站） |
...
data/afr*.json               阿里山林鐵：軌道幾何／OSM 缺口補線／時刻表
