...
## 四個系統(2026-07-08 接入 TDX)

| 系統 | 模式 | 資料 |
|---|---|---|
| 台北捷運 | 班距+TDX 真實站間/停站時間 | `data/trtc.json`(TDX 8 線含新北投/小碧潭支線;環狀線自 OSM 搬入) |
| 高雄捷運 | 班距+TDX 真實站間/停站時間 | `data/krtc.json`(紅/橘線+環狀輕軌 C 閉環;輕軌班距為公告估算) |
| 台中捷運 | 班距+TDX 真實站間/停站時間 | `data/tmrt.json`(綠線) |
| 台鐵 | 逐車次真實時刻表 | `data/tra_schedule_dense.json`(905 車次,2026-07-08;軌道幾何 `data/tra.json`) |

註:「台鐵(班距)」獨立模式已隨設計改版移除;`data/mrt.json`(OSM 北捷)僅作環狀線來源保留。

## 已完成功能

### 2026-07-09 夜間批次(依國內外鐵道迷網站研究挑選,見 docs/research-*.md)

- **跟隨路線高亮**:跟車時全程路徑描光暈(起點到終點),一眼看懂這班車要走哪。
- **旅程進度條**:跟隨面板顯示「已行/全程 km」(貼軌段用弧長,精度同軌跡)。
- **停靠站名牌**:停站時地圖上方淡入復古站名牌——本站大字+左右實際鄰站。
- **全程速度曲線**:資訊卡 FR24 式速度剖面圖,停站/加減速節奏一目了然+當前時刻游標。
- **旅程收集(完乘章)**:手動跟隨一班車到終點自動蓋章(參考日本「乗りつぶし」文化),
  🎫 面板統計完乘趟數/總里程/收集車種;放空與隨機模式不計。
- **放空模式日夜天色**:地圖色調隨模擬時刻連續變化(黎明橘→白晝→黃昏金→暮色紫→深夜藍)。
- **放空模式打磨**:進場直接貼車(z14)+自動 5×,換車淡出淡入(不再飛全島)。
- **車站分級圖示**:特等站紅框方牌、依站等遞減大小/顯示門檻;站名蓋在列車上層。
...
- **列車貼軌**:班距模式沿 shape 以里程前進(`posAlongShape`);真實班表模式把每個停靠站
  對到所屬線的里程(`assignSchedShapePaths`,28078 段貼軌 / 2112 段直線 ≈ 93%),
  實測行進中列車離軌 0 m。2026-07-08 `despike_shapes.mjs` 清除 OSM 尋路折返毛刺
  1235 個頂點+修正鳳鳴/烏日站序後,全 905 班取樣檢測倒退事件 0(原 5448)。
...
| `despike_shapes.mjs` | 清除 shape 折返毛刺(站場側線繞行)+重投影站點里程;fetch_shapes 後必跑 |
| `fetch_tdx.py` | TDX 捷運原始資料(北捷/高捷/高雄輕軌/中捷)→ `data/tdx/`(已抓 2026-07-08) |
| `build_tdx.mjs` | `data/tdx/*` → `trtc/krtc/tmrt.json`:碎片縫合、支線跨鏈接合、環線短弧、除毛刺、官方班距+站間/停站時間 |
...
- 捷運為班距模擬非逐班時刻;北捷/高捷/中捷的班距與站間時間取自 TDX 官方值,
  例外:環狀輕軌班距為公告估算、環狀線(Y)幾何與班距仍為 OSM+估算(`headway_estimated:true`)。

## 待辦(下一步)

1. 想做可做:高鐵、A→B 下一班查詢、匯出影片、北捷/高捷即時到站(TDX Live API)、
   環狀線改接新北捷運(NTMC)TDX 資料。
