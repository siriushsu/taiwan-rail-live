...
public class RailBoardWidgetProvider extends AppWidgetProvider {
    ...
    private static void updateOne(Context context, AppWidgetManager manager, int id) {
        ...
        try {
            ...
            manager.updateAppWidget(id, sizes(context, manager, id, snapshot, readable));
            ...
            if (fallback != null) {
                ...
                manager.updateAppWidget(id, sizes(context, manager, id, fallback, readable));
            } else {
                manager.updateAppWidget(id, tap(context, id,
    ...
    private static RemoteViews sizes(Context context, AppWidgetManager manager, int id, RailWidgetData.Snapshot snapshot,
        ...
        if (Build.VERSION.SDK_INT < 31) {
            ...
            return tap(at(context, family, snapshot, readable, size.getWidth(), size.getHeight()), tap);
        }
        ...
        for (SizeF size : candidates) {
            ...
            layouts.put(key, tap(at(context, tier, snapshot, readable, size.getWidth(), size.getHeight()), tap));
        }
    ...
    static RemoteViews at(Context context, String tier, RailWidgetData.Snapshot snapshot, boolean readable,
        ...
        RemoteViews preset = preset(context, tier, snapshot, readable);
    ...
    private static RemoteViews preset(Context context, String tier, RailWidgetData.Snapshot snapshot, boolean readable) {
        if (WidgetFamily.SMALL.equals(tier)) return small(context, snapshot, readable);
        if (WidgetFamily.LARGE.equals(tier)) return large(context, snapshot, readable);
        return medium(context, snapshot, readable);
    }
    ...
    /*
     * 三尺寸各一張版面。這裡的列數是「預設大小」（defaultSize）量出來的班數：桌面上的卡照實際高度重量（at），
     * 這個數只在量不出來時直接用，挑選器示範列也照它放。
     ...
     */
    static RemoteViews small(Context context, RailWidgetData.Snapshot snapshot, boolean readable) {
        return RailWidgetRender.board(context, R.layout.widget_rail_2x2, snapshot, 3, readable, true);
    }

    static RemoteViews medium(Context context, RailWidgetData.Snapshot snapshot, boolean readable) {
        return RailWidgetRender.board(context, R.layout.widget_rail_4x2, snapshot, 3, readable, false);
    }

    static RemoteViews large(Context context, RailWidgetData.Snapshot snapshot, boolean readable) {
