...
const native = {
  ...
  '不限方向': ['Any direction', '方向を指定しない'],
  '不指定（兩個方向都看）': ['Not set (show both directions)', '指定なし（両方向を表示）'],
  '只列出「{station}」開得到的方向': ['Only directions served from {station}', '{station}から行ける方向のみ表示'],
  ...
  '轉乘': ['Transfer', '乗換'], '轉乘 {station}': ['Transfer at {station}', '{station}で乗換'],
  '等候轉乘': ['Waiting for connection', '乗換待ち'],
  '{station} {time} 發車': ['{station} · departs {time}', '{station} {time}発'],
  '資料中斷・位置為預估': ['Data interrupted · position estimated', 'データ中断・位置は推定'],
