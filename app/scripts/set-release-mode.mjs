...
const MODES = {
  ...
  // 2026-07-30 更新：使用者裁示這一輪走 1.1.0 (15)——主畫面小工具是新功能，值得一個
  // minor 版號；build 跳到 15 是為了避開「1.0.3 (14) 那顆已作廢的 archive 是否已被 ASC
  // 吃掉」這個不確定性（那顆的刪帳號少了 device actor，不可送審）。下次要再改這張表之前，
  // 先去 ASC 確認號碼還空著——下面那道單調遞增閘門只擋「往回推」，擋不住「撞到已用過的號」。
  feature: {
    marketing: '1.1.0', build: '15', music: true,
