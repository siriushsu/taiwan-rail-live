...
const MODES = {
  ...
  // 線上修好不算）。
  feature: {
    // 2026-08-18：1.4.6 已於 08-16T17:16Z 上架（實查線上版號確認）⇒ 63 已燒、該條 train 關閉。
    // 這顆 64 是**真機試裝顆**（小工具改版第一次真的 xcodebuild widget extension，外加高鐵滑行入站模型）。
    // 要送審前 why 要補上高鐵滑行那條，現值只描述小工具改版。
    marketing: '1.4.7', build: '64', music: true, // 58–63 均已燒掉（63 已上架）
    why: '軌島 1.4.7：小工具版面改版。發車看板撤掉左側那條軌道示意——它會讓人以為上下兩列是同一條軌道上的前後站，實際上是不同車次；空出來的位置改放北上／南下方向三角與更大的終點站與倒數。台鐵發車看板新增大尺寸卡片（九列，並多一欄發車時刻），中尺寸從三班改四班，小尺寸改成兩班。另新增「大字好讀版」：字級整組放大、對比拉到 5:1 以上、列數相應減少，可在小工具設定裡開啟，系統字級調到輔助使用級別時也會自動切換。另修捷運小工具只放一張卡時換車站被誤要求訂閱。',
  },
