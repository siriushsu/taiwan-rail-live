...
const MODES = {
  ...
  feature: {
    marketing: '1.4.9', build: '77', music: true, metroCore: true, // 77＝76 全部＋地圖縮放/拖曳跨層同步修法(76 已上線,那顆缺這個修法)
    why: '軌島 1.4.9：台鐵車站看板可以挑一班車追蹤，鎖定畫面顯示它的表定時刻與官方誤點分鐘（實際約幾點到）。捷運小工具可一鍵開啟等車卡，北捷看板補上「再下一班・約 N 分」並區分官方倒數與推估分鐘，等車卡沿用同一套精度規則。承接統一捷運動畫與共站路線辨識，定位維持 App 前景持續更新。並修好地圖縮放與拖曳：兩指縮放放手時軌道不再先跳回原處再彈回來，拖曳地圖時軌道與列車不再比底圖慢半拍。',
  },
