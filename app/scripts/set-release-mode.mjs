...
const MODES = {
  ...
  feature: {
    marketing: '1.4.8', build: '68', music: true, // 58–67 均已燒掉（63 已上架；64/65/66/67 已上傳）
    why: '軌島 1.4.8：修好「站牌寫著列車要進站、地圖上卻沒有那台車」。台北捷運的官方逐車資料偶爾會整趟漏掉一列車，實測有一台往蘆洲的列車連續六分鐘都不在裡面，而各站的官方到站倒數一路都報得出它；現在只要官方倒數看得到某個車號，就照樣把車畫出來，行進與到站時刻和其他車完全一樣。另修好逐車資料漏填終點站時，車若在中和新蘆線兩條路線共用的那段軌道上會整台不顯示的問題。',
  },
