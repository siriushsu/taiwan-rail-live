...
const MODES = {
  ...
  feature: {
    marketing: '1.4.8', build: '65', music: true, // 58–64 均已燒掉（63 已上架、64 已上傳）
    why: '軌島 1.4.8：地圖底圖改用 OpenStreetMap 圖資（OpenFreeMap 向量圖磚），地名中文單行、路名與公路牌不再蓋滿畫面；來源可由伺服器遠端切換、載入異常時裝置自動退回原本的 Stadia 底圖，不必等出版本。台北捷運列車位置改回官方即時並修掉每十幾秒跳一下：車到站先停官方停靠秒數再照倒數開往下一站，倒數修訂只改速度不移位置；文湖線改由各站官方倒數推位置。另含 1.4.7 (64) 的小工具版面改版（發車看板撤軌脊、方向三角、Large、大字好讀版、自動選站服務範圍門檻）。',
  },
