...
const MODES = {
  ...
  feature: {
    marketing: '1.5.1', build: '82', music: true, metroCore: true, // 80=1.5.0 送審顆(基底 f30b4da=橫式 v5,與本機測試顆 907 同載貨)。ASC 線上 1.4.10 (79) 已發佈(08-23),train 已關,1.5.0 為新版本項目。出這顆時 HEAD 落後 origin/main 恰 1 顆=418fbe3(ship-web 網站部署鏈,不進 App bundle,刻意不併)。900 系照舊只裝自己 iPhone,不上傳 ASC。81 出過 IPA 但沒上傳:日文使用者更新完看到標著「中国語の原文」的中文更新說明(whyEn/whyJa 當時還沒有這條管線),且右下角版權字串在窄螢幕折四行蓋到速度鈕,82 兩件一起修

    why: '軌島 1.5.1\n\n三語真的切得動了\n上一版雖然做了繁中、English、日本語，iPhone 卻漏帶英、日文字典，按了語言鈕仍然全是繁中。這一版把三語資源完整帶進 App：介面、官方站名與路線名、車站與列車看板、設定、使用說明、圖鑑、旅程護照、成就與通行證都會一起切換；鎖定畫面的列車跟隨、捷運等車、台鐵等站，以及各種小工具也用同一種語言。動態島的展開畫面加了圓角安全留白，文字與「結束」按鈕不再被邊緣切掉。\n\n車廂擁擠度改成逐車對照\n忠孝復興這種兩條線共站的地方，不會再把板南線的擁擠度掛到文湖線那一列上；認不出是哪一台車就留白，不拿別台的數字頂替。三種車站看板現在都會顯示擁擠度，不會再切換一次就整片消失，鎖定畫面的等車卡也一起改。\n\n捷運信義東延段\n淡水信義線的終點由象山延伸到廣慈/奉天宮站，8/30 14 時通車。地圖上這一站與新路段已經先畫好，官方即時到站資料要等通車後才會帶到這一站。\n\n東部幹線急彎照官方核定速限\n宜蘭線新馬那個半徑 306 公尺的彎（2018 年出軌事故現場）普悠瑪 75、太魯閣 85、其他列車 65；北迴線和仁一帶非傾斜式列車 115。到站時刻完全不變——彎道慢下來的時間由同一段的其他部分吸收。\n\n其他\n開啟變快了：三十幾份路網與班表資料從一份一份載改成同時下載，重新造訪也不再重抓沒變動的檔案。即時資料連線中斷時的提示也改得更準確，不會再把「我這端或你的網路連不上」寫成營運單位的訊號中斷。',
    whyEn: 'Rail Island 1.5.1\n\nAll three languages now switch properly\nTraditional Chinese, English and Japanese were all prepared for the previous release, but the iPhone build shipped without the English and Japanese dictionaries, so pressing the language button left everything in Traditional Chinese. This release bundles all three languages in the app. The interface, official station and route names, the station and train boards, settings, Help, the Gallery, the Travel passport, achievements and the Pass all switch together, and Metro waiting, TRA station tracking and train following on the lock screen, along with the widgets, use the same language. Expanded Dynamic Island views also gained a rounded-edge safe margin, so text and the End button are no longer clipped.\n\nCarriage crowding is now matched train by train\nAt interchanges like Zhongxiao Fuxing, crowding from a Bannan line train no longer appears on the Wenhu line row, and a row that cannot be matched to a specific train is left blank rather than filled in with another train\'s figure. All three station board views now show crowding, so it no longer disappears when a board switches source, and the lock screen wait card was changed to match.\n\nTaipei Metro: Xinyi east extension\nThe Tamsui–Xinyi line extends one stop beyond Xiangshan to 廣慈/奉天宮, opening at 14:00 on 30 August. The station and the new stretch of track are already drawn on the map; official live arrival data reaches the new station once the line opens.\n\nOfficial speed limits on the eastern lines\nAt the 306 m radius curve by Xinma on the Yilan Line (site of the 2018 derailment), Puyuma runs at 75, Taroko at 85 and other trains at 65 km/h; non-tilting trains run at 115 km/h around Heren on the North Link Line. Arrival times are unchanged — the time lost through a curve is absorbed by the rest of the same run.\n\nOther changes\nThe app opens faster: the thirty-odd route and timetable files now download at the same time instead of one after another, and files that have not changed are not fetched again on a later visit. Outage notices are also more accurate: a problem at this service\'s end or with your own connection is no longer described as the operator\'s signal being down.',
    whyJa: '軌島 1.5.1\n\n3言語の切り替えが正しく動くようになりました\n前のバージョンでも繁体字中国語・英語・日本語を用意していましたが、iPhone版に英語と日本語の辞書が同梱されておらず、言語ボタンを押しても表示は繁体字中国語のままでした。このバージョンでは3言語のデータをすべてAppに収録します。インターフェース、公式の駅名・路線名、駅と列車の案内表示、設定、使い方、図鑑、旅のパスポート、実績、パスがまとめて切り替わり、ロック画面の列車追跡・メトロ待ち・台湾鉄路の駅待ちや、各種ウィジェットも同じ言語で表示されます。Dynamic Islandの展開表示には角丸の安全余白を追加し、文字や「終了」ボタンが端で欠けないようにしました。\n\n車両混雑度を列車ごとに照合するようにしました\n忠孝復興のような乗換駅で、板南線の混雑度が文湖線の行に表示されることはなくなりました。どの列車か特定できない行は、他の列車の数値で埋めずに空欄のままにします。3種類の駅発車案内すべてで混雑度を表示するようになり、表示元が切り替わっても消えなくなりました。ロック画面の待ち時間カードも同じ方式に変更しました。\n\n台北メトロ・信義線東延伸区間\n8月30日14時に信義線東延伸区間が開業し、淡水信義線の終点が象山から廣慈/奉天宮駅へ1駅延びます。地図には新駅と新しい線路をすでに描いていますが、公式のリアルタイム到着情報は開業後から新駅に届きます。\n\n東部幹線の急曲線を公式の速度制限に合わせました\n宜蘭線・新馬の半径306メートルの曲線（2018年の脱線事故現場）では、プユマが75キロ、タロコが85キロ、その他の列車が65キロで走行します。北廻線・和仁付近では、振り子式以外の列車が115キロで走行します。到着時刻は変わりません。曲線で失った時間は、同じ運転区間の他の部分で吸収します。\n\nその他\n起動時のデータ読み込みが速くなりました。30余りの路線・時刻表データを1件ずつではなく同時に読み込むようにし、次回以降は変更のないファイルを取得し直しません。リアルタイム情報が途切れたときの案内も実態に合わせ、軌島側やお使いの回線の問題を運行事業者の信号中断と表示することはなくなりました。',
  },
