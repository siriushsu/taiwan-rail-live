...
const MODES = {
  ...
  feature: {
    marketing: '1.4.5', build: '60', music: true, // 58＝提示之前那顆、59＝看板自成一格版(裁示改掉),都未上傳、號不重用
    why: '軌島 1.4.5：桌面／鎖定畫面的捷運小工具——選一個捷運站就能在主畫面看下一班倒數，另有「鐵路＋捷運看板」大卡把台鐵高鐵與捷運放在同一張；等車卡把某一站的倒數放上鎖定畫面與動態島，時間到自動收。免費可設一站，多站與「自動（最近的站）」屬軌島通行證。無新增權限與能力，只多一個 railisland:// 深連結供小工具點擊回 App。通行證的內容改成在用得到的地方直接講：跟車面板的鎖定畫面入口、切到衛星時說明高解析版本屬通行證；小工具與等車卡的用法寫進使用說明中心，與其他功能同一種說明方式。',
  },
