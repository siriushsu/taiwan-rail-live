...
const MODES = {
  ...
  feature: {
    ...
    why: '軌島 1.5.0\n\n繁中、English、日本語\n介面、官方站名與路線名、車站和列車看板、設定、使用說明、圖鑑、旅程護照、成就、通行證及重要隱私說明都能一起切換；選過的語言會保留。鎖定畫面的列車跟隨／等車資訊，以及台鐵、高鐵、捷運和雙看板小工具也會使用同一種語言。\n\n北捷列車改看官方倒數\n北捷的車現在照每一站的官方到站倒數反推位置——站牌上寫幾分，車就照那個時間到。以前用來定位的車廂編號站碼會落後兩三分鐘，已經不再拿它畫位置。\n只要官方還在報這班車的倒數，它就會在畫面上，不會因為系統「判斷不出來」而突然消失；車開到終點站才收起來，而且從早上一發車就認得它。折返站的車不再原地打轉或憑空多出一台，位置也不再一跳一跳。\n\n看得更舒服\n整個介面重新排過：\n・字級新增標準／大／特大三階，也可以跟著系統字級走\n・手機頂端收成單排，右邊一顆鈕就能切換全台／台鐵／高鐵／捷運\n・點時鐘就知道現在的資料是不是即時的，以及為什麼\n・面板可以切成半透明，底下的路線與車站看得到\n・跟車途中點車站，看板會和跟車卡併成同一張卡的兩個分頁；點地圖空白處卡片縮成膠囊，跟車不中斷\n・列車詳細資訊一個捲軸從摘要接到停靠表\n・列車到站時的站名牌縮小了，不再擋住後面的資訊\n・橫放有專屬版面\n\n其他\n放空模式的背景音樂從 29 首增加到 57 首，分成六組不同氛圍。搜尋、最愛、旅程護照與今日亮點一併重整；空白的畫面會告訴你下一步可以做什麼；站內可以直接回報問題或建議。',
    // 英日整段文案(2026-08-30 補):Android 線的「更新了什麼」原本三語都吐這段中文,
    // 與 iOS 線同一個根因。內容是本版(1.5.0)中文 why 的對譯,不是 iOS 1.5.1 那份。
    whyEn: 'Rail Island 1.5.0\n\nTraditional Chinese, English and Japanese\nThe interface, official station and route names, the station and train boards, settings, Help, the Gallery, the Travel passport, achievements, the Pass and the key privacy notices all switch together, and the language you pick is remembered. Train following and waiting information on the lock screen, along with the TRA, THSR, metro and dual-board widgets, use the same language.\n\nTaipei Metro trains now follow the official countdown\nMetro train positions are now derived from each station\'s official arrival countdown, so a train arrives at the time the platform sign says it will. The carriage-number station code previously used for positioning runs two to three minutes behind, and is no longer used to place trains.\nAs long as the operator is still reporting a countdown for a train, that train stays on screen; it no longer vanishes because the app cannot work out what it is. A train is only taken off once it reaches its terminus, and it is recognised from its first departure of the morning. Trains at turnback stations no longer spin in place or appear twice, and positions no longer jump.\n\nEasier to read\nThe whole interface has been laid out again:\n- Three text sizes (standard, large, extra large), or follow the system size\n- The phone header is now a single row, with one button on the right to switch between All Taiwan, TRA, THSR and metro\n- Tap the clock to see whether the data is live, and why\n- Panels can be made translucent so the routes and stations underneath stay visible\n- Tapping a station while following a train merges the board and the following card into two tabs of one card; tapping an empty part of the map shrinks the card to a capsule without interrupting following\n- Train details scroll from the summary through to the stop list in one column\n- The station nameplate shown when a train arrives is smaller and no longer covers what is behind it\n- Landscape has a layout of its own\n\nOther changes\nAmbient mode\'s background music grew from 29 tracks to 57, in six different moods. Search, favourites, the Travel passport and today\'s highlights were reorganised; empty screens now say what you can do next; and problems or suggestions can be reported from inside the app.',
    whyJa: '軌島 1.5.0\n\n繁体字中国語・英語・日本語\nインターフェース、公式の駅名・路線名、駅と列車の案内表示、設定、使い方、図鑑、旅のパスポート、実績、パス、主要なプライバシー説明がまとめて切り替わり、選んだ言語は保存されます。ロック画面の列車追跡・待ち時間表示や、台湾鉄路・台湾高鉄・メトロ・2画面ウィジェットも同じ言語で表示されます。\n\n台北メトロの列車を公式カウントダウンから描くようにしました\n台北メトロの列車位置は、各駅の公式到着カウントダウンから逆算するようになりました。ホームの案内が示す時刻どおりに列車が到着します。これまで位置決めに使っていた車両番号の駅コードは2〜3分遅れるため、位置の描画には使わなくなりました。\n公式がその列車のカウントダウンを出している限り、列車は画面に残ります。アプリが「判定できない」せいで突然消えることはありません。終点に着いたときに初めて取り下げ、朝の始発から同じ列車として認識します。折り返し駅の列車がその場で回り続けたり、二重に現れたりすることもなくなり、位置が飛び跳ねることもなくなりました。\n\n見やすくなりました\nインターフェース全体を組み直しました：\n・文字サイズに標準・大・特大の3段階を追加し、システムの文字サイズに追従させることもできます\n・スマートフォンの上部を1行にまとめ、右端のボタン1つで全台湾・台湾鉄路・台湾高鉄・メトロを切り替えられます\n・時計をタップすると、いま表示しているデータがリアルタイムかどうかと、その理由が分かります\n・パネルを半透明にでき、下の路線や駅が見えたままになります\n・列車を追跡中に駅をタップすると、発車案内と追跡カードが1枚のカードの2つのタブにまとまります。地図の何もない場所をタップするとカードはカプセル状に縮み、追跡は途切れません\n・列車の詳細は、概要から停車駅一覧まで1つのスクロールでつながります\n・列車が到着したときの駅名表示を小さくし、後ろの情報を隠さないようにしました\n・横向き専用のレイアウトを用意しました\n\nその他\n鑑賞モードのBGMを29曲から57曲に増やし、6つの雰囲気に分けました。検索・お気に入り・旅のパスポート・本日のハイライトも整理し、内容が空の画面では次にできることを案内します。アプリ内から不具合や提案を直接報告できます。',
  },
