...
const MODES = {
  ...
  // 2026-08-14：build 50＝**1.4.3**。這張表停在 1.4.2 (44) 而 pbxproj 早已是 1.4.3 (49)
  ...
  //    build 從 50 起：45–49 五顆 1.4.3 的 ipa 都已產出（~/Desktop），無法從本機確認
  //    哪些已被 ASC 吃掉，往上跳一號的成本是零、撞號的成本是退件。
  //    內容＝48 的旋轉尺寸過期自癒（P0）再加上北捷官方即時名冊（origin/main dab284a 併入）。
  feature: {
    marketing: '1.4.3', build: '50', music: true,
    why: '軌島 1.4.3：台北捷運列車改以官方站牌倒數逐班追蹤，從發車到終點依各站時間顯示位置，同班資訊自動合併，車次與方向跟著實際行程；保留橫放版面、背景音樂續播與 App 更新提示；production 資格，TestFlight Sandbox 通道維持關閉。',
  },
