...
const MODES = {
  ...
  // 2026-08-14：build 49＝**1.4.3**。這張表停在 1.4.2 (44) 而 pbxproj 早已是 1.4.3 (48)
  //    ——單調遞增閘門會直接擋下（正是它該做的事），補齊到當前這一輪。
  //    marketing 維持 1.4.3：lookup API 08-14 實查線上仍是 `1.4.2`（照腳本自己的告誡實查，
  //    不採信筆記），1.4.3 的版本紀錄尚未送出，換掉底下的 build 即可。
  //    build 從 49 起：45–48 四顆 1.4.3 的 ipa 都已產出（~/Desktop），無法從本機確認
  //    哪些已被 ASC 吃掉，往上跳一號的成本是零、撞號的成本是退件。
  //    內容＝48 的旋轉尺寸過期自癒（P0）再加上北捷官方即時名冊（origin/main dab284a 併入）。
  feature: {
    marketing: '1.4.3', build: '49', music: true,
    why: '軌島 1.4.3：台北捷運改用官方即時資訊決定車子在哪、依車站看板時刻到站（不再對不上班表就消失）；手機橫放過之後地圖尺寸過期導致看不到車的問題自動修復；沿用 1.4.2 的橫放版面、App 更新提示與評分、轉乘提示、附近車站與隨機跟隨互斥，以及背景音樂續播、鎖屏自動換站、高鐵自由座車廂、我的車・準點、準點排行；production 資格，TestFlight Sandbox 通道維持關閉。',
  },
