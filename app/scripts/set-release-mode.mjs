...
const MODES = {
  ...
  feature: {
    // 2026-08-17：1.4.6 已於 08-16T17:16Z 上架（lookup 帶 no-cache 實查）⇒ 63 已燒、該條 train 關閉，
    // 行銷版號推進到 1.4.7。這顆 64 是**小工具改版的真機試裝顆**（第一次真的 xcodebuild widget extension）。
    // ⚠️ 要送審前 why 必須重寫：現值仍是 1.4.5 那批的文案，還沒描述改版與換站修法。
    marketing: '1.4.7', build: '64', music: true, // 58–63 均已燒掉
    why: '軌島 1.4.5：桌面／鎖定畫面的捷運小工具——選一個捷運站就能在主畫面看下一班倒數，另有「鐵路＋捷運看板」大卡把台鐵高鐵與捷運放在同一張；等車卡把某一站的倒數放上鎖定畫面與動態島，時間到自動收。免費可設一站，多站與「自動（最近的站）」屬軌島通行證。無新增權限與能力，只多一個 railisland:// 深連結供小工具點擊回 App。通行證的內容改成在用得到的地方直接講：跟車面板的鎖定畫面入口、切到衛星時說明高解析版本屬通行證；小工具與等車卡的用法寫進使用說明中心，與其他功能同一種說明方式。另修兩件捷運顯示：中和新蘆線共用幹線上同一班車被蘆洲／迴龍兩支各認一次而畫成兩台，以及切到沒有捷運的群組時「官方中斷 N 分」會一路往上爬的假警報（收班時段改標「已收班」）。',
  },
