...
export function verifyAndroidWidgetParity({ log = true } = {}) {
  ...
  const contentRules = [
    ...
    [`widget_rail_2x2 示範列數＝${railSmallRows}（預設大小列數＝${exp2x2.rows}，RailBoardWidgetProvider.java board() maxRows）`,
      railSmallRows === exp2x2.rows],
    [`widget_rail_4x2 示範列數＝${railMediumRows}（預設大小列數＝${exp4x2.rows}，RailBoardWidgetProvider.java board() maxRows）`,
      railMediumRows === exp4x2.rows],
    [`widget_rail_4x4 示範列數＝${railLargeRows}（預設大小列數＝${exp4x4.rows}，RailBoardWidgetProvider.java board() maxRows）`,
