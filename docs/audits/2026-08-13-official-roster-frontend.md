...
- 開旗標且 `feedMode === 'official'`、`vehicles` 合法、`sourceRevision` 未超過 45 秒時，`boardPos.vehicles` 是唯一畫車母體。一筆 vehicle 畫一台；`rows`、`trips` 或班表均不增減這個母體。payload 逐次整包原子換入，ID 重複、站序越界、方向逆行、非相鄰移動、移動列 `run<=0`或 `depEpoch>=arrEpoch`、欄位壞掉均整體 fail closed 到 outage，不會逐車靜默漏畫。
...
- `drawFreq`、全台同框 `drawDeco`、畫面計數、命中與跟隨共用同一份官方 render items。`feedMode=outage`、資料 malformed、倒退或 age >45 秒時，render items 回 `null`，整體才落回既有班表路徑；不逐線混補班表車。
...
mtime 不是 payload 契約，只用來估計 edge/cache 額外延遲。若以 client age 判：15 秒會誤退 75/100 輪，20 秒誤退 7/100，30／45／60 秒均為 0/100；30 秒只比觀測 max 多 1.559 秒。使用者裁示採 **45 秒**：每秒以 `Date.now()/1000-sourceRevision` 重算，`<=45` 保留官方名冊，超過即整體班表備案；不是只在 fetch 當刻算一次。
...
- **客端時鐘**：45 秒 gate 以 `Date.now()` 對官方來源時刻；裝置時鐘超前會提早 fallback，落後會延後 stale 退場。目前未另加 future-skew 下界。
- **成功空 frame**：fresh `vehicles=[]` 是權威空名冊；現有100輪沒有整包空樣本。若上游故障卻 HTTP 200 持續回合法空列，僅靠 age 無法與真空班分辨。
- **凍結但 HTTP 200**：若上游持續重播舊列，sourceRevision 不前進會在45秒退場；若錯誤地同步刷新 `NowDateTime`，本層無法辨識內容凍結。
...
- **模擬時間**：官方路徑刻意採 wall clock；拖動時刻尺時，北捷官方車與台鐵／其他 fallback 車的 sim time 可能脫軸。這是「即時優先」裁示的直接取捨。
