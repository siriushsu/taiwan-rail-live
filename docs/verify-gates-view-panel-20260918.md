...
| verify_sunlight_controls | 車站卡＋觀看面板仍可操作 | **產品行為（待裁）** | 開著車站卡時 `#viewSettingsBtn` 不可見；舊版「更多」在底部分頁列，開車站卡仍可達 | 待使用者決定：恢復「開著車站卡也能進觀看設定」，或改判準 |
| verify_sunlight_controls | 分享連結 `sun=on` 覆蓋本次偏好 | 未定 | 關閉偏好跨重載那條有過，`&sun=on` 後 `sunlight.enabled` 為假 | 單獨重現 |
