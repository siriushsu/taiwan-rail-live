...
| 組 | 項數 | 原因 | 引入點 | 處置 |
|---|---|---|---|---|
| L1b 放大後列車在露出中心＋前瞻 | 48 | 產品行為改變（相機取捨） | a5473792 v0909i 手機車頭鎖定 | **未動，待使用者裁示** |
| P4 小卡態／46% sheet 列車在露出中心＋前瞻 | 4 | 同上 | 同上 | **未動，待使用者裁示** |
...
| L10 分組切換（收合鈕）按得動 | 2（僅 WebKit） | **產品回歸（效能）** | f14ee875 v0915a 同向待避選站 | 判準不改（它抓到的是真的），只補診斷欄；產品修法待決定。紅幾顆看負載（修後全套 small 3002 ms 紅、medium 2724 ms 擦邊過） |
...
- 修法（待決定，不在這一輪）：待避結果依資料日快取、切組別不重算；或把計算移出點擊當下。
