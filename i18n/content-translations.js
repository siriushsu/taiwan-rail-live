(() => {
  ...
  Object.assign(messages.en, {
    ...
    "最遠征": "Longest trip",
    "非即時": "Not live",
    "台鐵誤點目前非即時：依 {time} 的即時資料投射 · {n} 班誤點中": "TRA delays are not live: projected from the {time} feed · {n} trains delayed",
    "找不到就檢查一下車次是不是今天的班次——停駛或加班車不一定在時刻表裡。": "If nothing matches, check whether the train runs today — cancelled and extra services may not be in the timetable.",
    ...
    "桌": "Widget",
    "字級": "Text size",
    "三階字級與跟隨系統字級。": "Three text sizes, plus follow-the-system option.",
    ...
    "「跟隨系統字級」可以關掉——手機字調大、但這一頁想維持資訊密度時用": "“Follow system text size” can be turned off — useful when your phone is set large but you want this page dense",
    "「大」只放大字級與列高，功能位置完全不變；跟隨系統只在字級進到輔助使用級別才介入。": "“Large” only increases text and row height; nothing moves. Following the system only kicks in at accessibility sizes.",
    ...
    "從選單挑全台同框／台鐵／高鐵／捷運與輕軌": "Pick All together / TRA / HSR / Metro and light rail from the menu",
    "桌面：頂端資訊列上的四顆分頁直接點": "Desktop: tap one of the four tabs in the top information bar",
    "切換不會重新框景——你原本看的地方還在原地。字級調大時那顆鈕會跟著變大，但位置不變。": "Switching does not reframe the map — you stay where you were. The button grows with larger text, but does not move.",
    "點時鐘＝現在的資料是不是即時的": "Tap the clock = is the data live right now",
    "時鐘旁邊就是資料狀態——畫面夠寬時直接寫出來（LIVE、非即時、尖峰…），窄的時候依序收成小色點，「即時」那顆最後才收。點一下時鐘，會逐項寫出每一項代表什麼、以及為什麼是這個狀態。": "Data status sits next to the clock — spelled out when there is room (LIVE, Not live, Peak…), and collapsed into coloured dots when there is not, with “live” the last to collapse. Tap the clock to see what each one means and why it is in that state.",
    "點頂端的時鐘（桌面點時鐘那張卡）": "Tap the clock at the top (on desktop, the clock card)",
    "逐列看：台鐵即時誤點是 LIVE 還是「非即時」、捷運看板校正到哪、是不是在重播、目前時段與車數": "Row by row: whether TRA live delays are LIVE or “Not live”, how far the metro boards have been corrected, whether you are replaying, and the current period and train count",
    ...
    "想放多站，或用「自動（最近的站）」讓它跟著你移動換站，需要軌島通行證（免費可設定一站）": "Multiple stations, or “Automatic (nearest station)” that follows you as you move, need a Rail Island Pass (one station is free)",
    ...
    "在月台上等車時，把這一站的下一班倒數放上鎖定畫面與動態島，不用一直把 App 開著。": "While you wait on the platform, put this station’s next-train countdown on the Lock Screen and Dynamic Island instead of keeping the app open.",
    "點北捷、高捷或機捷的車站打開看板": "Tap a Taipei Metro, Kaohsiung Metro or Airport MRT station to open its board",
    ...
    "在月台等某一班車時，把那一班的倒數放上鎖定畫面與動態島，不用一直把 App 開著。": "While you wait on the platform for a particular train, put that train’s countdown on the Lock Screen and Dynamic Island instead of keeping the app open.",
    ...
    "倒數照官方表定加即時誤點，車到站後自動收起。鎖定畫面同時只留一張等候卡——追蹤台鐵這班會把捷運那張收掉。時鐘不在「現在」時會先幫你帶回現在。這項不需要通行證。": "The countdown follows the official timetable plus live delays, and closes itself once the train arrives. Only one waiting card stays on the Lock Screen at a time — tracking this TRA train closes the metro one. If the clock is away from “now”, it is brought back first. No pass required.",
    "App 留在前景時藍點與所在地鏡頭會持續跟著你；「附近車站」告訴你最近的車站與下一班車還有幾分鐘。": "While the app stays in the foreground, the blue dot and the location camera keep following you; “Nearby stations” tells you the closest station and how many minutes until the next train.",
    ...
    "手動拖圖會解除所在地鏡頭，定位藍點仍持續更新": "Dragging the map releases the location camera; the blue dot keeps updating",
    ...
    "點地圖空白處小卡會收成一顆膠囊——跟車不會斷，點膠囊就展開回來": "Tapping an empty part of the map collapses the card into a pill — following does not stop, and tapping the pill expands it again",
    ...
    "附上的只有版本、目前畫面、地圖視角、跟隨中的列車與瀏覽器版本，不含帳號與定位。開 issue 需要 GitHub 帳號，issue 內容是公開的。": "Only the version, current view, map viewport, the train being followed and the browser version are attached — no account or location. Opening an issue needs a GitHub account, and issues are public.",
    "讓看板、列車卡與跟隨面板透出後方地圖，面板底下的路線、車站都看得到；面板最上方的標題列也一起透。": "Lets the boards, train cards and following panel show the map behind them, so the lines and stations underneath stay visible — the title bar at the top of the panel is translucent too.",
    "預設關閉；這項設定只會記在目前這台裝置。半透明時字色會自動調整以維持可讀性（亮色加深、暗色提亮），不是靠在字外加光暈。": "Off by default, and remembered only on this device. When translucent, text colour is adjusted automatically to stay readable — darker on light, lighter on dark — rather than adding a glow around the text.",
    "通行證內容：台鐵列車的誤點履歷（回溯 90 天的逐日紀錄）、iPhone 桌面與鎖定畫面的捷運小工具放多站或用「自動（最近的站）」跟著你移動換站（免費可設定一站）、收藏與完乘紀錄跨裝置雲端同步、行程分享、在 App 匯入 Google Maps 已儲存清單、App 非跟車時的衛星高解析圖磚，以及 iOS 17.6 以上可用的跟車鎖定畫面與動態島即時動態。": "What the pass includes: delay history for TRA trains (day-by-day records going back 90 days); metro widgets on the iPhone Home and Lock Screen with multiple stations or “Automatic (nearest station)” that follows you as you move (one station is free); cloud sync of favourites and ride records across devices; trip sharing; importing your saved Google Maps lists in the app; high-resolution satellite tiles when not following a train; and Lock Screen and Dynamic Island Live Activities while following a train, on iOS 17.6 or later.",
  });
  Object.assign(messages.ja, {
    "{n} 班車": "{n}本の列車",
    "{n} 個地點": "{n}か所",
    ...
    "切換：全台／台鐵／高鐵／捷運": "切り替え：全国／台湾鉄路／高鉄／メトロ",
    ...
    "從選單挑全台同框／台鐵／高鐵／捷運與輕軌": "メニューから全国まとめて／台湾鉄路／高鉄／メトロ・ライトレールを選ぶ",
    "桌面：頂端資訊列上的四顆分頁直接點": "デスクトップ：上部の情報バーにある4つのタブをタップ",
    "切換不會重新框景——你原本看的地方還在原地。字級調大時那顆鈕會跟著變大，但位置不變。": "切り替えても地図は再フレーミングされず、見ていた場所のままです。文字を大きくするとボタンも大きくなりますが、位置は変わりません。",
    "點時鐘＝現在的資料是不是即時的": "時計をタップ＝いまのデータがリアルタイムかどうか",
    "時鐘旁邊就是資料狀態——畫面夠寬時直接寫出來（LIVE、非即時、尖峰…），窄的時候依序收成小色點，「即時」那顆最後才收。點一下時鐘，會逐項寫出每一項代表什麼、以及為什麼是這個狀態。": "データの状態は時計の隣にあります。幅に余裕があれば文字で（LIVE、リアルタイムではありません、ピークなど）、狭いときは順に小さな色の点にまとまり、「リアルタイム」が最後まで残ります。時計をタップすると、各項目の意味と現在の状態の理由が1行ずつ表示されます。",
    "點頂端的時鐘（桌面點時鐘那張卡）": "上部の時計をタップ（デスクトップでは時計のカード）",
    "逐列看：台鐵即時誤點是 LIVE 還是「非即時」、捷運看板校正到哪、是不是在重播、目前時段與車數": "行ごとに確認：台湾鉄路の遅延がLIVEか「リアルタイムではありません」か、メトロ案内の補正状況、再生中かどうか、現在の時間帯と列車数",
    ...
    "台鐵套用官方即時誤點；北捷依官方秒級到站倒數逐班校正，高捷、機捷、高雄輕軌依官方到站看板校正。官方資料中斷或你的裝置時鐘明顯不準時，上方的「LIVE」會轉成灰色「非即時」，表示畫面是依稍早的資料投射——點一下時鐘可以看到是哪一項出狀況。台鐵當日官方公告停駛的班次不會出現在地圖上，但看板仍會列出並標成「停駛」。": "台湾鉄路は公式のリアルタイム遅延を反映します。台北メトロは公式の秒単位カウントダウンで1本ずつ補正し、高雄メトロ、桃園空港MRT、高雄ライトレールは公式の到着案内で補正します。公式データが途切れたり端末の時計が大きくずれている場合、上部の「LIVE」が灰色の「リアルタイムではありません」に変わり、少し前のデータからの推定表示であることを示します。時計をタップするとどれが原因か分かります。当日公式に運休となった台湾鉄路の列車は地図には出ませんが、案内には「運休」と表示されます。",
    ...
    "還有一張「鐵路＋捷運看板」大卡，把台鐵、高鐵與捷運的發車資訊列在同一張。點小工具會直接開啟軌島到那一站。": "台湾鉄路・高鉄・メトロの発車情報を1枚にまとめた大きな「鉄道＋メトロ案内」カードもあります。ウィジェットをタップすると、その駅で軌島が開きます。",
    ...
    "倒數照官方表定加即時誤點，車到站後自動收起。鎖定畫面同時只留一張等候卡——追蹤台鐵這班會把捷運那張收掉。時鐘不在「現在」時會先幫你帶回現在。這項不需要通行證。": "カウントダウンは公式時刻表に実際の遅延を加えたもので、列車が到着すると自動的に閉じます。ロック画面に残る待機カードは同時に1枚だけで、この台湾鉄路の列車を追跡するとメトロのカードは閉じます。時計が「現在」からずれている場合は、先に現在へ戻します。パスは不要です。",
    ...
    "手動拖圖會解除所在地鏡頭，定位藍點仍持續更新": "地図をドラッグすると現在地カメラは解除されますが、青い点は更新され続けます",
    "按「附近車站」回到目前位置，點清單裡的站直接開看板": "「近くの駅」で現在地に戻り、一覧の駅をタップすると案内が開きます",
    "點地圖空白處小卡會收成一顆膠囊——跟車不會斷，點膠囊就展開回來": "地図の空いている場所をタップするとカードはピル状に畳まれます。追跡は止まらず、ピルをタップすると元に戻ります",
    "真的要結束按卡片上的「結束」": "本当に終了するときはカードの「終了」を押します",
    "按小卡上的「結束」停止跟車(× 只是把卡收起來,車還在跟)": "カードの「終了」で追跡を停止します（×はカードを畳むだけで、追跡は続いています）",
    "跟到終點會蓋一枚完乘章（收在「護照」）。加速播放沒問題，但中途跳時間就不算。手機在跟車途中點車站，看板會和跟車小卡併成同一張卡的兩個分頁，不會互相蓋掉。": "終点まで乗り通すと完乗スタンプがもらえます（「パスポート」に保存）。早送り再生は問題ありませんが、途中で時間を飛ばすと対象外です。スマートフォンで追跡中に駅をタップすると、案内と追跡カードが1枚のカードの2つのタブにまとまり、互いに隠れません。",
    "看到不對的地方，可以帶著當下的畫面資訊回報。": "おかしいところがあれば、いまの画面の情報を添えて報告できます。",
    "按頁尾（手機在「更多」的關於段）的「回報問題或建議」": "フッターの「問題や提案を報告」をタップ（スマートフォンでは「その他」の概要セクション内）",
