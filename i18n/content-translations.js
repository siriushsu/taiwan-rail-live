(() => {
  ...
  Object.assign(messages.en, {
    ...
    '整段列車轉公車旅程現在可以用短效連結分享：對方會看到正在搭列車、走去站牌、等車或搭公車；手機位置預設不分享，只有你另行勾選才會在 App／網頁留在前景時更新，停止分享會連同位置立即刪除。': 'Share the whole rail-to-bus journey with a short-lived link. The recipient sees whether you are on the train, walking to the stop, waiting or aboard the bus. Phone location is off by default, updates only after a separate opt-in while the app or page is in the foreground, and is deleted when sharing stops.',
  ...
  Object.assign(messages.ja, {
    ...
    '整段列車轉公車旅程現在可以用短效連結分享：對方會看到正在搭列車、走去站牌、等車或搭公車；手機位置預設不分享，只有你另行勾選才會在 App／網頁留在前景時更新，停止分享會連同位置立即刪除。': '鉄道からバスまでの旅程全体を短時間リンクで共有できます。相手には列車乗車中、停留所へ徒歩移動中、バス待ち、バス乗車中の状態が表示されます。端末の位置共有は既定でオフで、別途有効にした場合だけApp／ページが前面にある間更新し、共有停止時に位置も削除します。',
