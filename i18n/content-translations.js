(() => {
  ...
  Object.assign(messages.en, {
    ...
    "手機地圖右下角的版權資訊不會再折成一大塊蓋住速度調整鈕：改成貼著底緣一整條、放不下的部分橫向可捲，授權連結一個都沒少，仍然看得到也點得到。切到衛星影像時字串較長，原本折成四行最嚴重，現在一樣是一條（謝謝網友回報）": "The map attribution in the bottom-right corner no longer wraps into a block that covers the speed control. It now sits along the bottom edge as a single line, and anything that does not fit scrolls sideways — every licence link is still there, still visible and tappable. It was worst in satellite mode, where the longer string wrapped onto four lines; that is a single line now too. (Thanks to the reader who reported it.)",
    ...
    "英文介面把字級調到大或特大時，左上角的軌島招牌會被切掉一截、甚至畫到牌子外面；現在牌會自己縮到裝得下，換系統的按鈕也不再被擠出畫面": "With the interface in English and the text size set to large or extra large, the Rail Island nameplate in the top-left corner was cut off, and could even spill outside the plate itself. The plate now shrinks to fit, and the button for switching systems is no longer pushed off screen.",
  ...
  Object.assign(messages.ja, {
    ...
    "手機地圖右下角的版權資訊不會再折成一大塊蓋住速度調整鈕：改成貼著底緣一整條、放不下的部分橫向可捲，授權連結一個都沒少，仍然看得到也點得到。切到衛星影像時字串較長，原本折成四行最嚴重，現在一樣是一條（謝謝網友回報）": "地図右下の著作権表示が折り返して速度調整ボタンを覆うことがなくなりました。画面下端に沿った1行になり、収まらない部分は横スクロールで読めます。ライセンスのリンクは1つも減っておらず、表示もタップもできます。文字列が長い衛星画像モードでは4行に折り返すのが最も深刻でしたが、こちらも1行になりました。（ご報告いただいた方に感謝します。）",
    ...
    "英文介面把字級調到大或特大時，左上角的軌島招牌會被切掉一截、甚至畫到牌子外面；現在牌會自己縮到裝得下，換系統的按鈕也不再被擠出畫面": "英語表示で文字サイズを「大」または「特大」にすると、左上の軌島の看板が途中で切れたり、看板の外にはみ出して描画されたりしていました。看板が収まる大きさまで自ら縮むようにし、路線網を切り替えるボタンも画面外に押し出されなくなりました。",
