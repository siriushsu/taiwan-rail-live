(() => {
  ...
  Object.assign(messages.en, {
    ...
    "英文與日文的手機頂列不會再把分組切換鈕擠出畫面：英文版在 360 寬的手機上，原本「捷運」那一顆整顆看不到、也沒有地方可以捲過去；現在左上角的軌島牌裝不下時會自己讓位，拉丁字母也不再套漢字招牌的字距": "On phones, the English and Japanese top bar no longer pushes the group tabs off screen. In English on a 360 dp phone the Metro tab used to be completely out of view with nowhere to scroll to it; the nameplate now gives up space when it does not fit, and Latin text no longer uses the letter spacing meant for the Chinese plate.",
  ...
  Object.assign(messages.ja, {
    ...
    "英文與日文的手機頂列不會再把分組切換鈕擠出畫面：英文版在 360 寬的手機上，原本「捷運」那一顆整顆看不到、也沒有地方可以捲過去；現在左上角的軌島牌裝不下時會自己讓位，拉丁字母也不再套漢字招牌的字距": "英語・日本語のスマートフォン上部バーが、グループ切り替えタブを画面外に押し出さなくなりました。英語では360dpの端末で「メトロ」タブが完全に見えず、スクロールする場所もありませんでしたが、名板が収まらないときは自ら場所を譲るようにし、ラテン文字には漢字の看板用の字間を適用しないようにしました。",
