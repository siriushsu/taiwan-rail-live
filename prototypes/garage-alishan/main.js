...
let journey,follow3D,pose,renderer,environment,primary,train,coast,raf=0,last=0,time=0,distance=0,period='day',view=matchMedia('(max-width:800px)').matches?'train':'world',running=!reduced.matches,zoom=1,yaw=-1.35,elevation=.67,disposed=false,ready=false,draws=0;
...
function draw(){
 ...
 focus.copy(target);
 const span=(view==='train'?Math.max(9,train.length*.65/aspect):Math.max(29,43/aspect))/zoom;
...
function reset(){zoom=1;yaw=-1.35;elevation=view==='train'?.48:.67;controls();schedule();}
...
try{
 ...
 coast=createScene();scene.add(coast.group);primary=await loadGarageModel('dl38');if(disposed){primary.dispose();throw Error('disposed');}train=await createConsist('dl38',primary);if(disposed){train.dispose();throw Error('disposed');}scene.add(train.root);train.root.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});
