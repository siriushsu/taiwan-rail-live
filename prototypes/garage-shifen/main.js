...
import {createScene,THEMES} from '../../rail-3d/garage-scenes/shifen.js';
...
const SPEED=1.7; // 每秒沿路徑前進的弧長；setTime(t) 與逐幀播放共用同一個係數，兩者永遠一致。
let follow3D,renderer,environment,primary,train,shifen,raf=0,last=0,time=0,distance=0,period='day',view=matchMedia('(max-width:800px)').matches?'train':'world',running=!reduced.matches,zoom=1,yaw=-1.12,elevation=.58,disposed=false,ready=false,draws=0;
const scene=new THREE.Scene(),camera=new THREE.OrthographicCamera(-40,40,30,-30,.1,400);camera.up.set(0,0,1);
const focus=new THREE.Vector3(),target=new THREE.Vector3(),sun=new THREE.DirectionalLight('#fff1cf',3.2),hemi=new THREE.HemisphereLight('#c1dce7','#7b8663',2.1);
...
function draw(){
 ...
 shifen.update(time,period);follow3D(shifen.path,distance);
 const rect=canvas.getBoundingClientRect(),aspect=rect.width/Math.max(1,rect.height);
 if(view==='train'){target.set(0,0,0);for(const c of train.cars)target.add(c.car.position);target.multiplyScalar(1/train.cars.length);target.z+=1.4;}else target.set(0,1.0,1.2);
 focus.copy(target);
 const span=(view==='train'?Math.max(9,train.length*.65/aspect):Math.max(24,35/aspect))/zoom;
...
function frame(at){raf=0;if(disposed||document.hidden)return;if(last&&at-last<32){schedule();return;}const dt=last?Math.min((at-last)/1000,.08):0;last=at;if(running){distance+=dt*SPEED;time+=dt;}draw();if(running)schedule();}
function reset(){zoom=1;yaw=-1.12;elevation=view==='train'?.5:.58;controls();schedule();}   // 平地老街的跟車鏡頭抬高一點（.42→.5），近排店屋的屋頂才不會蓋掉車身下半
...
// 老街中段正好在 s=0（shifen.js 的 sample(0) 落在前直線中點，店屋兩排之間）；停在這裡不需要另外找錨點。
document.querySelector('#platform').onclick=()=>{running=false;distance=0;last=0;if(view!=='world')setView('world');else{controls();schedule();}};  // 看老街一律切回全景，兩排店屋與燈籠串才看得到
...
canvas.onpointerdown=e=>{if(e.button>0)return;canvas.setPointerCapture(e.pointerId);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});gesture=null;};
canvas.onpointermove=e=>{if(!pointers.has(e.pointerId))return;const old=pointers.get(e.pointerId);pointers.set(e.pointerId,{x:e.clientX,y:e.clientY});if(pointers.size===1){yaw+=(e.clientX-old.x)*.007;elevation=Math.max(.2,Math.min(1.2,elevation+(old.y-e.clientY)*.005));schedule();}else{const [a,b]=[...pointers.values()],gap=Math.hypot(a.x-b.x,a.y-b.y);if(gesture)setZoom(gesture.zoom*gap/gesture.gap);else gesture={zoom,gap:Math.max(1,gap)};}};
...
canvas.onkeydown=e=>{if(!['ArrowLeft','ArrowRight','ArrowUp','ArrowDown','+','-','0',' '].includes(e.key))return;e.preventDefault();if(e.key===' ')document.querySelector('#play').click();else if(e.key==='0')reset();else if(e.key==='+')setZoom(zoom*1.2);else if(e.key==='-')setZoom(zoom/1.2);else{if(e.key==='ArrowLeft')yaw-=.15;if(e.key==='ArrowRight')yaw+=.15;if(e.key==='ArrowUp')elevation=Math.min(1.2,elevation+.1);if(e.key==='ArrowDown')elevation=Math.max(.2,elevation-.1);schedule();}};
...
function dispose(){if(disposed)return;disposed=true;ready=false;cancelAnimationFrame(raf);observer.disconnect();train?.dispose();primary?.dispose();shifen?.dispose();environment?.dispose();groundGeo.dispose();groundMat.dispose();sun.shadow.map?.dispose();renderer?.dispose();}
...
try{
 ...
 shifen=createScene();scene.add(shifen.group);primary=await loadGarageModel('dr1000');if(disposed){primary.dispose();throw Error('disposed');}train=await createConsist('dr1000',primary);if(disposed){train.dispose();throw Error('disposed');}scene.add(train.root);train.root.traverse(o=>{if(o.isMesh){o.castShadow=true;o.receiveShadow=true;}});
