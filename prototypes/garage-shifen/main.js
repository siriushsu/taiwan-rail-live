...
let follow3D,renderer,environment,primary,train,shifen,raf=0,last=0,time=0,distance=0,period='day',view=matchMedia('(max-width:800px)').matches?'train':'world',running=!reduced.matches,zoom=1,span=1,yaw=-1.12,elevation=.58,disposed=false,ready=false,draws=0;
...
function draw(){
 ...
 span=(view==='train'?Math.max(9,train.length*.65/aspect):Math.max(24,35/aspect))/zoom;
...
function reset(){zoom=1;pan.set(0,0,0);yaw=-1.12;elevation=view==='train'?.5:.58;controls();schedule();}   // 平地老街的跟車鏡頭抬高一點（.42→.5），近排店屋的屋頂才不會蓋掉車身下半
