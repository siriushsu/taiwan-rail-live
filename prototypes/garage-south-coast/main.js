...
let renderer,environment,primary,train,coast,raf=0,last=0,time=0,distance=0,period='day',view=matchMedia('(max-width:800px)').matches?'train':'world',running=!reduced.matches,direction=1,zoom=1,yaw=-1.14,elevation=.65,disposed=false,ready=false,draws=0;
...
function controls(){document.querySelector('#play').textContent=running?'Ⅱ':'▷';document.querySelector('#play').setAttribute('aria-label',running?'暫停行駛':'開始行駛');document.querySelector('#play').setAttribute('aria-pressed',String(running));document.querySelector('#reverse').setAttribute('aria-pressed',String(direction<0));document.querySelector('#in').disabled=zoom>=1.8;document.querySelector('#out').disabled=zoom<=.7;}
...
function draw(){
 ...
 coast.update(time,period);train.follow(coast.path,distance,1); // 反向只改弧長速度，不翻轉／瞬移編組。
 ...
 focus.copy(target);
 const span=(view==='train'?Math.max(9,train.length*.65/aspect):Math.max(23,39/aspect))/zoom;
 ...
 canvas.dataset.ready='true';canvas.dataset.direction=String(direction);canvas.dataset.distance=String(distance);canvas.dataset.period=period;canvas.dataset.view=view;
}
function frame(at){raf=0;if(disposed||document.hidden)return;if(last&&at-last<32){schedule();return;}const dt=last?Math.min((at-last)/1000,.08):0;last=at;if(running){distance+=dt*2.1*direction;time+=dt;}draw();if(running)schedule();}
function reset(){zoom=1;yaw=-1.14;elevation=view==='train'?.38:.65;controls();schedule();}
...
document.querySelector('#reverse').onclick=()=>{direction*=-1;controls();schedule();};document.querySelector('#reset').onclick=reset;
