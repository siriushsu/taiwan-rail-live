...
import {createScene,THEMES} from '../../rail-3d/garage-scenes/viaduct.js';
...
const SPEED=2.1; // 每秒沿路徑前進的弧長；setTime(t) 與逐幀播放共用同一個係數，兩者永遠一致。
let follow3D,renderer,environment,primary,train,viaduct,raf=0,last=0,time=0,distance=0,period='day',view=matchMedia('(max-width:800px)').matches?'train':'world',running=!reduced.matches,zoom=1,yaw=-1.12,elevation=.58,disposed=false,ready=false,draws=0;
...
function reset(){zoom=1;yaw=-1.12;elevation=view==='train'?.42:.58;controls();schedule();}
...
// 月台中心正好在 s=0（viaduct.js 的 sample(0) 落在前直線中點、面對月台）；停在這裡不需要另外找錨點。
document.querySelector('#platform').onclick=()=>{running=false;distance=0;last=0;if(view!=='world')setView('world');else{controls();schedule();}};  // 跟車視角看月台會被雨棚擋死，一律切回全景
