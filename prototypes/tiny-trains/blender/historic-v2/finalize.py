...
for s in sorted(specs,key=lambda v:v['priority']):
 ...
 files={f.name:{'bytes':f.stat().st_size,'sha256':hashlib.sha256(f.read_bytes()).hexdigest()} for f in p.iterdir() if f.is_file()}
...
(P/'release.json').write_text(json.dumps({'version':2,'status':'plan-calibration-review','count':len(assets),'mapEligible':sum(json.loads((P/'models'/a['id']/'model.json').read_text()).get('mapEligible',False) for a in assets),'pendingGeoreference':sum(not json.loads((P/'models'/a['id']/'model.json').read_text()).get('mapEligible',False) for a in assets),'date':'2026-09-09','assets':assets,'implementationSHA256':{name:hashlib.sha256((P/name).read_bytes()).hexdigest() for name in ['build_calibrated.py','export_core.py','geometry.py','calibration.py','placements-source.json','catalog-source.json','viewer.html','viewer.mjs','serve.mjs']},'verificationSHA256':{f.name:hashlib.sha256(f.read_bytes()).hexdigest() for f in (P/'verification').glob('*.json')}},ensure_ascii=False,indent=2)+'\n')
