...
export async function buildingCatalog(){
 const catalogs=await Promise.all(collections.map(async collection=>{
  const base=new URL('./assets/'+collection+'/',import.meta.url);
  const read=async name=>{const r=await fetch(new URL(name,base));if(!r.ok)throw Error('建築模型資料載入失敗 '+name);return r.json();};
  const [catalog,placement]=await Promise.all([read('catalog.json'),read('placement.json')]);
  return Promise.all(catalog.map(async entry=>{const source=await read(entry.id+'/model.json'),p=placement.entries[entry.id];if(!p?.anchor||source.railElevationM!=null||p.railElevationM!==null)throw Error('建築定位契約不符 '+entry.id);return {base,entry,meta:{...source,anchor:p.anchor,rotationDeg:p.rotationDeg,displayHeightM:source.sizeM[2],railElevationM:null,blender:true},source,placement:p,footprint:p.footprint};}));
