// 林冠為 OSM 林地內的示意植被，不宣稱單株位置。只有兩個 instanced draw calls；
...
export function createLandscapeTrees({map,THREE,scene,world,clearance,getTerrain}){
  ...
  let timer=null,disposed=false,movedAt=0,revision=-1,dirty=true,firstDirty=performance.now();
  const stats={count:0,cap,rebuilds:0,maxBuildMs:0,coordinates:[],checks:0};
  const layers=['landcover_wood'];
  const color=new THREE.Color();
  function rebuild(){
    if(disposed||document.hidden)return;
    if(map.getZoom()<14.5){crowns.count=trunks.count=stats.count=0;stats.coordinates=[];return;}
    const start=performance.now(),c=map.getCenter(),radius=1000,bounds=map.getBounds();
    const features=map.queryRenderedFeatures({layers:layers.filter(id=>map.getLayer(id))});
    const obstacles=map.queryRenderedFeatures({layers:['water','building','highway_path','highway_minor','highway_major_inner'].filter(id=>map.getLayer(id))});
    ...
    outer:for(const f of features){
      ...
      for(const rings of polys){
        ...
        for(let x=Math.floor(w*MX/STEP);x<=Math.ceil(e*MX/STEP);x++)for(let y=Math.floor(s*MY/STEP);y<=Math.ceil(n*MY/STEP);y++){
          ...
          seen.add(key);items.push({p,r,seed,height,d:Math.hypot((p[0]-c.lng)*MX,(p[1]-c.lat)*MY)});
        }
      }
    }
    ...
    stats.coordinates=selected.map(({p})=>p);stats.checks=checks;stats.rebuilds++;stats.maxBuildMs=Math.max(stats.maxBuildMs,performance.now()-start);map.triggerRepaint();
  }
  ...
  function schedule(e){if(disposed||e?.sourceId&&!['openmaptiles','terrain'].includes(e.sourceId))return;if(e?.sourceId&&!e.tile&&e.sourceDataType!=='content')return;
