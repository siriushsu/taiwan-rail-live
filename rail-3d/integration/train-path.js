...
export function makePath(coordinates,loop=false){
  ...
  function locate(coord,hint){let best=null;const mx=Math.cos(coord[1]*Math.PI/180)*Math.PI/180*EARTH_M,my=Math.PI/180*EARTH_M;
    for(let i=1;i<coordinates.length;i++){if(Number.isFinite(hint)&&(d[i]<hint-350||d[i-1]>hint+350))continue;const a=coordinates[i-1],b=coordinates[i],x=(coord[0]-a[0])*mx,y=(coord[1]-a[1])*my,dx=(b[0]-a[0])*mx,dy=(b[1]-a[1])*my,q=dx*dx+dy*dy;if(!q)continue;const t=Math.max(0,Math.min(1,(x*dx+y*dy)/q)),error=Math.hypot(x-t*dx,y-t*dy),s=d[i-1]+t*(d[i]-d[i-1]);
