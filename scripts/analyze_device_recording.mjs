...
// 量每一幀「底圖引擎畫的洋紅環」與「overlay 畫的青點」圓心距離。圓心用已知半徑的圓擬合(probe_centroids.mjs):
// 探針被列車卡片/HUD/路線壓掉一塊仍量得到;圓周看得見不到 35% 就擬合失敗。洋紅環的內外兩圈要同心才算「確定」的圓心;
// 只擬合得出一圈且可見弧 <50%、或青點可見弧 <50% 的擬合是「不確定」的——超標時不算 over,算 weakOver(受 unsure-share 管)。
//
// 每一幀分成七類(設計書 3.2「探針找不到＝該格失敗」的落地口徑,09-04 裁定):
//   ok          兩顆都量到、距離 ≤ 上限
//   over        兩顆都量到(兩顆都確定)、距離 > 上限               → 一格就 FAIL,零容忍
//   weakOver    兩顆都量到但至少一顆不確定、距離 > 上限            → 不列判定,受 unsure-share 管(直邊相切的假圓心長這樣)
...
const DEFAULTS = {
  fps: Number(opt('--fps', 30)), dpr: Number(opt('--dpr', 3)), threshold: Number(opt('--threshold', 2)), // 抽幀率:真機錄 60fps,10 只看 1/6、單幀失步六次抓一次;要保證看到每一幀用 --fps 60
  ...
  maxUnsureShare: Number(opt('--max-unsure-share', 0.1)), // orphanNear＋orphanBlind＋weakOver 占抽幀總數的上限:「判不準」的幀能容忍多少
  probe: opt('--probe', 'ring'), // 錄影時探針的幾何:ring(v0904l 起,洋紅環 12–18css)或 disc(之前的實心圓;沒有內圈可驗,只能單圈擬合)
};
...
export async function analyze(video, out, o = {}) {
  const { fps, dpr, threshold, minMeasured, coverage, maxGapSec, maxBlindSec, maxBlindShare, maxUnsureShare, probe } = { ...DEFAULTS, ...o };
  ...
  const radii = { magR: 18 * dpr, magInR: probe === 'disc' ? null : 12 * dpr, cynR: 5 * dpr }; // index.html 的探針:ALIGN_RING_PAINT 環 12–18css、overlay ctx.arc 5css
  const edgeMargin = Math.max(radii.magR, radii.cynR) + limitPx; // 找到的那顆離邊緣不到這麼近,另一顆被裁掉就是合理的
  // 「附近」＝另一顆若真畫在同一處,它的色塊最遠能離找到那顆的圓心多遠:另一顆的半徑＋判準上限。09-04 稽核抓到舊值
  // 2×洋紅半徑(dpr3 → 36 裝置點)是上限的 18 倍,青點偏 30 裝置點仍被當「同一處」。找到青、洋紅被遮的那一面幾何上仍只能
  // 分辨到約(洋紅半徑－青半徑)的偏移,所以另有 orphan-share 閘門封頂「判不出來」的幀數。
  const nearR = { mag: radii.cynR + limitPx, cyn: radii.magR + limitPx }; // 找到洋紅→青的色塊要在 nearR.mag 內;找到青→洋紅要在 nearR.cyn 內
  const rows = [];
  for (const f of files) {
    ...
    if (c.mag && c.cyn) { d = Math.hypot(c.mag.x - c.cyn.x, c.mag.y - c.cyn.y); kind = d <= limitPx ? 'ok' : (c.mag.sure && c.cyn.sure ? 'over' : 'weakOver'); }
      ...
      else if (colorMassNear(data, info.width, info.height, otherTest, found.x, found.y, c.mag ? nearR.mag : nearR.cyn) >= MIN_PIX) kind = 'orphanNear';
      else kind = 'orphanBlind';
    }
    rows.push({ frame: f, mag: c.mag, cyn: c.cyn, dist: d, kind });
  }
  const kinds = { ok: 0, over: 0, weakOver: 0, absent: 0, orphanEdge: 0, orphanNear: 0, orphanBlind: 0 };
...
function printReport(r) {
  ...
  console.log(`  影格分類: ok=${r.kinds.ok} over=${r.kinds.over} weakOver=${r.kinds.weakOver} absent=${r.kinds.absent} orphanEdge=${r.kinds.orphanEdge} orphanNear=${r.kinds.orphanNear} orphanBlind=${r.kinds.orphanBlind}`);
...
//   mag/cyn:畫不畫那顆(洋紅是 12–18 的環,同 index.html 的 ALIGN_RING_PAINT);cynDx:青點右移;occlude:深色矩形從圓心下方 3px 蓋到底(兩顆都只剩上半);
//   cx:圓心 x(負值＝貼左邊緣);cover:[{x,y,w,h}] 額外的深色矩形(做「遮到擬合不出、但顏色還在」的孤兒)。
const CX = 195, CY = 422;
async function synth(dir, name, frames, srcFps = 10) {
  const fdir = path.join(dir, name + '-frames'); rmSync(fdir, { recursive: true, force: true }); mkdirSync(fdir, { recursive: true });
  const W = 390, H = 844;
  let i = 0;
  for (const fr of frames) {
    const { mag = true, cyn = true, cynDx = 0, occlude = false, cx = CX, cy = CY, cover = [], lines = [] } = fr;
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${W}" height="${H}"><rect width="${W}" height="${H}" fill="#e8e8e8"/>` +
      lines.map(l => `<line x1="${l.x1}" y1="${l.y1}" x2="${l.x2}" y2="${l.y2}" stroke="${l.color}" stroke-width="${l.w}"/>`).join('') + // 正式線色的軌道,畫在探針底下
      (mag ? `<circle cx="${cx}" cy="${cy}" r="15" fill="none" stroke="#ff00ff" stroke-width="6"/>` : '') +
  ...
  run('ffmpeg', ['-y', '-v', 'error', '-framerate', String(srcFps), '-i', path.join(fdir, 'f_%03d.png'), '-pix_fmt', 'yuv420p', '-c:v', 'libx264', f]);
  return f;
}
...
async function selftest() {
  ...
  const cases = [
    ...
    // 洋紅只剩中間橫帶:兩圈都擬合不出(→orphanNear),或外圈被帶子直邊釣出一個不確定的假圓心(→weakOver);兩種都不准假紅、也不准算 over
    ['near-orphan', [...A(60), ...A(5, NEAR)], r => r.pass && r.kinds.over === 0 && r.kinds.orphanNear + r.kinds.weakOver === 5 && r.kinds.ok === 60],
    ...
    ['blind-persistent', [...A(60), ...A(25, { cyn: false }), ...A(40)], r => !r.pass && only(r, 'max-blind-run', 'blind-share', 'unsure-share') && r.kinds.orphanBlind === 25],
    ...
    // 09-04 稽核 P0:遮到擬合不出的那顆其實偏了——舊口徑(附近＝兩倍洋紅半徑)當成「同一處」全綠。青偏 12px 幾何上仍分到
    // orphanNear／weakOver(洋紅只剩中間橫帶,判不出確定的圓心),靠 unsure-share 封頂才會紅;紅的必須恰好是那一條、不准是 over
    // 環的近側被偏掉的青點自己蓋住時洋紅色塊會少到分成 orphanBlind(blind-share 也紅):兩種紅都對,考的是「不准算 over、也不准綠」
    ['near-orphan-desync', [...A(60), ...A(20, { ...NEAR, cynDx: 12 })], r => !r.pass && r.kinds.over === 0 && failedGates(r).includes('unsure-share') && failedGates(r).every(g => g === 'unsure-share' || g === 'blind-share') && r.kinds.orphanNear + r.kinds.weakOver + r.kinds.orphanBlind === 20],
    // 正式線色撞探針色(沙崙線 #B565A7、內灣線 #00A0B0 落在舊的三條不等式門檻裡、屏東線 #8E44AD 的鋸齒邊會跨過):畫在探針旁不能搶走擬合
    ['line-colors', A(20, { lines: [{ x1: 0, y1: CY + 40, x2: 390, y2: CY + 40, color: '#B565A7', w: 9 }, { x1: CX + 60, y1: 0, x2: CX + 60, y2: 844, color: '#00A0B0', w: 9 }, { x1: 0, y1: CY - 40, x2: 390, y2: CY - 40, color: '#8E44AD', w: 9 }] }), r => r.pass && r.measured === 20 && r.mean < 1.5],
    // 單幀失步:60fps 來源只錯一格,分析也用 60fps 才保證看得到(預設 30 只看一半、10 只看 1/6)
    ['spike-60fps', [...A(30), ...A(1, { cynDx: 27 }), ...A(30)], r => !r.pass && only(r, 'over') && r.kinds.over === 1, { srcFps: 60, fps: 60 }],
  ];
  let ok = true;
  for (const [name, frames, expect, o = {}] of cases) {
    const r = await analyze(await synth(out, name, frames, o.srcFps || 10), path.join(out, name), { dpr: 1, threshold: 2, fps: o.fps || 10 });
    printReport(r);
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  if (args.includes('--selftest')) await selftest();
    ...
    if (!video) { console.error('用法:node scripts/analyze_device_recording.mjs <影片.mp4> [--fps 30(單幀失步要 60)] [--dpr 3] [--threshold 2] [--coverage 0.6] [--max-gap 3] [--max-blind 2] [--max-blind-share 0.05] [--max-unsure-share 0.1] [--probe ring|disc(v0904l 之前的錄影)] [--min-measured 10] [--out 目錄]\n      node scripts/analyze_device_recording.mjs --selftest'); process.exit(2); }
