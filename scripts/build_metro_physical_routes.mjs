...
for(const st of Object.values(stations)){
 ...
 // OSM 已標示的 stop_position 是月台實際停車點，不能再用官方站心向軌道投影覆蓋。
 // 同名轉乘站可能屬於另一條線（南港展覽館同時有文湖線與板南線），所以還要限制在
 // 目前路線線形的 45 m 走廊內，且沿線里程距官方站心不超過 100 m。
 const officialByGroup=new Map(g.stopCandidates(st,st.system).filter(c=>{const hit=path.locate(c.coordinate,s);return hit&&hit.error<=45&&Math.abs(hit.s-s)<=100;}).map(c=>[c.trackGroup,c]));
 ...
 st.candidates=[];for(const c of [...byGroup.values()].sort((a,b)=>a.score-b.score).slice(0,6)){const trackGroup=st.system+':'+c.group,official=officialByGroup.get(trackGroup);let nodeId=official?.nodeId,coordinate=official?.coordinate||c.coordinate;if(!nodeId&&c.t<1e-7)nodeId=c.e.a;else if(!nodeId&&c.t>1-1e-7)nodeId=c.e.b;else if(!nodeId){
   const key=c.e.id+':'+Math.round(c.t*1e8);nodeId=inserted.get(key);if(!nodeId){nodeId='metro-stop:'+key;inserted.set(key,nodeId);data.nodes[nodeId]=coordinate;data.nodeTags[nodeId]={railway:'stop',name:st.name,_source:'estimated-stop-on-osm-track'};if(!insertions.has(c.e.wayId))insertions.set(c.e.wayId,[]);insertions.get(c.e.wayId).push({edgeIndex:+c.e.id.split(':').at(-1),t:c.t,id:nodeId});inferred.push({id:nodeId,station:st.key,sourceWay:c.e.wayId,sourceSegment:[c.e.a,c.e.b],fraction:c.t,coordinate});}
  }st.candidates.push({nodeId,trackGroup,coordinate});
 }
}
