...
const SYSTEMS = [
  ...
  { file: 'data/sanying.json', out: 'data/sanying_times.json', estimated: true,
    src: '三鶯線免費試營運期(2026-06-30~08-31)無公開逐班時刻表:依新北捷運公司公告營運時段08:00-22:00與班距(平日尖峰08:00-08:30、17:30-19:30 6分/平日離峰及假日8分)合成,非公告時刻',
    // ⏰ 會過期的值,到期必重查 https://www.ntmetro.com.tw/basic/?mode=detail&node=863
    //   2026-08-01 起延長為 08:00-22:00(原 10:00-20:00 錯開早通勤,通勤族反映後調整)。
    //   官方站 node=863/10165 當日仍掛舊文字,依據取自新北捷運公司對外發布、口徑一致的報導:
    //   自由時報 2026-07-31「從早上八點營運至晚間十點」「平日上下班尖峰時段(早上八點至八點半、
    //   下午五點半至晚間七點半)列車班距約為六分鐘」「平日離峰及假日全天時段,維持約八分鐘班距」;
    //   壹蘋 2026-07-30 同口徑。三個已知的到期訊號:
    //   (1) 2026-08-31 免費試營運結束,全日營運日期官方稱「另行公告」;
    //   (2) 官方站 node=863/10165 補上新文字時,以官方文字為準覆蓋本處;
    //   (3) 官方稱「視現場人潮機動加開列車」→ 班距是公告值,不等於實際發車。
    //   在官方改點前寫成 06:00-23:30 正是 v0711j 的錯——把「正式營運後」的規劃當成現況,
    //   每天生出 7.5 小時不存在的幽靈列車(使用者 2026-07-18 回報)。
    synth: [{ lineId: 'LB', cfg: (() => {
      const OFF = 480, PEAK = 360, first = toSec('08:00'), last = toSec('22:00');
      // 平日雙尖峰(08:00-08:30 早、17:30-19:30 晚)6 分;假日全天 8 分,無尖峰
      return { services: {
        '平日': { first, last, bands: [[first, toSec('08:30'), PEAK], [toSec('08:30'), toSec('17:30'), OFF],
