...
fs.writeFileSync('rail-3d/physical/level-profiles.json',JSON.stringify({version:1,railElevationM:null,basis:'OSM 明示 bridge/tunnel + layer 交叉上下序 + 固定 DEM；地面初值 0m，橋隧 8m 層位初值、7m 相交淨距、8% 顯示過渡限制均為估計，不是工程標高。',inputSha256:Object.fromEntries(['network.json','metro-network.json','display-profiles.json','metro-display-profiles.json','../terrain/manifest.json'].map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync('rail-3d/physical/'+f)).digest('hex')])),sources:['https://www.futsu.com.tw/p_transportation.html','https://wiki.openstreetmap.org/wiki/Key:layer','https://web.metro.taipei/pages2026/WebStation/051'],solver:{iterations,violation,crossings:crossings.length},entries}));console.log({ways:Object.keys(entries).length,counts,iterations,violation});
