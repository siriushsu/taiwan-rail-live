...
const crossings=collectLevelCrossings(records);
// ── 顯示層位：用 tier，不用 rank ────────────────────────────────────
...
const flat=applyFlatRailGrade(records,entries,crossings);console.log({flat});
