...
const read=f=>JSON.parse(fs.readFileSync('rail-3d/physical/'+f)),number=v=>typeof v==='string'&&/^-?\d+(\.\d+)?$/.test(v)?Number(v):null;
function classify(t={}){const layer=number(t.layer),bridge=t.bridge&&t.bridge!=='no',passage=['avalanche_protector','building_passage'].includes(t.tunnel),kind=passage&&!bridge?'surface':t.tunnel==='yes'||t.location==='underground'?'tunnel':bridge?'bridge':layer<0?'tunnel':layer>0?'bridge':'surface',rank=kind==='tunnel'?Math.min(-1,layer??-1):kind==='bridge'?Math.max(1,layer??1):0;return {kind,rank,layer};}
const records=[],segments=[],grid=new Map(),seen=new Set(),crossings=[];
for(const [nf,pf]of [['network.json','display-profiles.json'],['metro-network.json','metro-display-profiles.json']]){const n=read(nf),p=read(pf);for(const w of n.ways){const path=makePath(w.coordinates),e=p.entries[w.id],c={...classify(w.tags),...(rankOverrides[w.id]!==undefined?{rank:rankOverrides[w.id],correction:'來源 level=B1；與 layer=-3 衝突，採與相鄰 B1 股道一致的樓層序'}:{})},at=s=>{let i=0,j=e.distances.length-1;while(j-i>1){const k=(i+j)>>1;if(e.distances[k]<=s)i=k;else j=k;}const f=(s-e.distances[i])/(e.distances[j]-e.distances[i]||1);return e.values[i]*(1-f)+e.values[j]*f;},r={w,path,e,c,at,pins:[]};records.push(r);
...
for(const r of records){const {w,path,e,c}=r;const snap=s=>{let lo=0,hi=path.d.length-1;while(hi-lo>1){const m=(lo+hi)>>1;if(path.d[m]<=s)lo=m;else hi=m;}return Math.abs(path.d[lo]-s)<.001?path.d[lo]:Math.abs(path.d[hi]-s)<.001?path.d[hi]:s;};for(const p of r.pins)p.s=snap(p.s);
 ...
 for(const s of distances){const key=source.get(s)||w.id+':s:'+s;let id=byKey.get(key);if(id===undefined){id=nodes.length;byKey.set(key,id);nodes.push({z:0,count:0,ground:-Infinity});}const node=nodes[id];node.z+=c.rank*8;node.count++;node.ground=Math.max(node.ground,r.at(s));r.ids.push(id);}
...
const entries={},counts={};for(const r of records){const offsets=r.ids.map(i=>+nodes[i].z.toFixed(4));if(offsets.every(z=>Math.abs(z)<.0001))continue;const values=r.ids.map(i=>+(nodes[i].ground+nodes[i].z).toFixed(4));entries[r.w.id]={...r.c,distances:r.distances,offsets,values};counts[r.c.kind]=(counts[r.c.kind]||0)+1;}
fs.writeFileSync('rail-3d/physical/level-profiles.json',JSON.stringify({version:1,railElevationM:null,basis:'OSM bridge/tunnel/layer + 固定 DEM；8m 層位初值、7m 相交淨距、8% 顯示過渡限制均為估計，不是工程標高。',inputSha256:Object.fromEntries(['network.json','metro-network.json','display-profiles.json','metro-display-profiles.json'].map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync('rail-3d/physical/'+f)).digest('hex')])),sources:['https://wiki.openstreetmap.org/wiki/Key:layer','https://web.metro.taipei/pages2026/WebStation/051'],solver:{iterations,violation,crossings:crossings.length},entries}));console.log({ways:Object.keys(entries).length,counts,iterations,violation});
