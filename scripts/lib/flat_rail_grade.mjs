...
export const FLAT_BASIS='平坦地圖使用 flatOffsets：全網共用節點，以 200m 尺度柔化短橋與引道，再限制露天顯示坡度（臺鐵高鐵 2.5%、林鐵 6%、捷運 4%），保留 7m 交叉淨距，露天段不因柔化而被地下段拉低至地面以下。地下疊層維持原 8% 約束；上述均為顯示估計，不是實測標高。';
export function applyFlatRailGrade(records,entries,crossings){
 ...
 for(const r of records){
  ...
  r.flatIds=e.distances.map((s,i)=>{const key=source.get(s.toFixed(4))||r.w.id+':s:'+s;let id=keys.get(key);if(id===undefined){id=nodes.length;keys.set(key,id);nodes.push({mass:0,target:0,floor:-Infinity});}if(e.kind!=='tunnel')nodes[id].floor=Math.max(nodes[id].floor,Math.min(0,e.offsets[i]));return id;});
