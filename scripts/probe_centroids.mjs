...
// 已知半徑 r 的圓擬合:RANSAC(兩個邊緣點決定兩個候選圓心)取內點最多者,再定半徑精修。
// 回 { x, y, n: 內點數, arc: 看得見的圓周比例 } 或 null。
export function fitCircle(pts, r) {
  ...
  for (let it = 0; it < iters; it++) {
    ...
    for (const s of [1, -1]) {
      ...
      let n = 0; for (const p of pts) if (Math.abs(Math.hypot(p[0] - cx, p[1] - cy) - r) <= RIM_TOL) n++;
      if (n > bestN) { bestN = n; best = [cx, cy]; }
    }
  }
  if (!best || bestN < MIN_ARC * circ) return null;
  let [cx, cy] = best;
  ...
  return { x: cx, y: cy, n: bestN, arc: bestN / circ };
}
