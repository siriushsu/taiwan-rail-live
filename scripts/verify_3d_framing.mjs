...
for(const [engine,type]of Object.entries(process.env.ENGINE==='chromium'?{chromium}:{chromium,webkit})){
 ...
 try{
  ...
  await p.waitForFunction(()=>window.railIslandIntegration?.renderer&&state.followTrain,null,{timeout:60000});await p.evaluate(()=>{state.playing=false;setSimSec(43200);M.raw.setZoom(17);});await p.waitForFunction(()=>railIslandIntegration.renderer.stats.models>=1,null,{timeout:30000});
  const select=async(sys,direction)=>{
   ...
   await p.waitForFunction(id=>railIslandIntegration.renderer.stats.poseSamples.some(s=>s.id===id),chosen.id);
   return chosen;
  };
  const side=async(offset,zoom)=>{
   await p.waitForFunction(()=>railIslandIntegration.renderer.stats.poseSamples.some(s=>s.id===railIslandIntegration.frame.selectedVehicleId));
   await p.evaluate(({offset,zoom})=>{const ri=railIslandIntegration,r=ri.renderer,s=r.stats.poseSamples.find(s=>s.id===ri.frame.selectedVehicleId),bearing=90-s.cars[0].angle*180/Math.PI+offset;r.map.jumpTo({zoom,pitch:55,bearing});},{offset,zoom});await p.waitForTimeout(80);await p.waitForFunction(()=>!railIslandIntegration.renderer.stats.followReturning);
  ...
  await p.tap('#tabMore');await p.locator('[data-rail3d="formation"] [data-value="three"]').scrollIntoViewIfNeeded();await p.tap('[data-rail3d="formation"] [data-value="three"]');await p.waitForFunction(()=>railIslandIntegration.renderer.stats.poseSamples[0]?.carCount===3);await p.waitForTimeout(400);
  check(engine+' / 三節示意維持原構圖',await p.evaluate(()=>railIslandIntegration.renderer.stats.followFraming.distanceM===0));
  await p.tap('[data-rail3d="formation"] [data-value="actual"]');await p.waitForFunction(()=>railIslandIntegration.renderer.stats.poseSamples[0]?.carCount===12);await p.waitForTimeout(200);
