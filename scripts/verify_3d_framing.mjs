...
for(const [engine,type]of Object.entries(process.env.ENGINE==='chromium'?{chromium}:{chromium,webkit})){
 ...
 try{
  ...
  // 真正使用「觀看」面板（v0914c 起 3D 設定從更多搬過去）點選三節與完整編組。
  if(await p.locator('.view-rail [data-view="train"]').isVisible())await (l=>l.tap())(p.locator('.view-rail [data-view="train"]'));else{await (l=>l.tap())(p.locator('#viewSettingsBtn'));await (l=>l.tap())(p.locator('.view-tabs [data-view="train"]'));}await p.locator('[data-rail3d="formation"] [data-value="three"]').scrollIntoViewIfNeeded();await p.tap('[data-rail3d="formation"] [data-value="three"]');await p.waitForFunction(()=>window.railIslandIntegration?.renderer?.stats.poseSamples[0]?.carCount===3);await p.waitForTimeout(400);
  ...
  check(engine+' / 完整編組恢復向車頭構圖',await p.evaluate(()=>railIslandIntegration.renderer.stats.followFraming.distanceM>0));await p.tap('.view-close');
