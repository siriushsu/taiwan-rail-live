...
for(const [engineName,engine]of Object.entries(process.env.ENGINE==='chromium'?{chromium}:{chromium,webkit})){
 ...
 try{
  ...
  await page.evaluate(()=>M.raw.setZoom(12));await page.waitForFunction(()=>window.railIslandIntegration?.renderer?.stats.models===0);check(engineName+' 遠景回到原車號',await page.evaluate(()=>state._trainHits.some(h=>h.tr===state.followTrain)));
  await page.evaluate(()=>M.raw.setZoom(17));await page.waitForFunction(()=>window.railIslandIntegration?.renderer?.stats.models>0);
  await page.evaluate(()=>{railIslandIntegration.setFormationMode('three');});await page.waitForFunction(()=>window.railIslandIntegration?.renderer?.stats.poseSamples[0]?.carCount===3);check(engineName+' 三節可切換',true);
  await page.evaluate(()=>railIslandIntegration.setFormationMode('actual'));await page.waitForFunction(()=>window.railIslandIntegration?.renderer?.stats.poseSamples[0]?.carCount===12);
