...
for(const[name,engine]of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  await page.waitForFunction(()=>!window.railIslandIntegration?.loading&&window.railIslandIntegration?.renderer?.stats.models>0);
  ...
  for(const kind of ['sched','metro']){
   ...
   await page.waitForFunction(id=>window.railIslandIntegration?.renderer?.hasModel(id),id);await page.waitForTimeout(500);
