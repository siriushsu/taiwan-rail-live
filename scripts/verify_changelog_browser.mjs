  ...
  for (const [engineName, engine] of Object.entries({ chromium, webkit })) {
    ...
    try {
      for (const width of widths) {
        ...
        if (historyCount !== 247) failures.push(`${engineName} ${width}px 完整歷史為 ${historyCount} 條`);
...
console.log(`更新紀錄瀏覽器驗收通過：Chromium + WebKit；${widths.join('/')}px 觸控寬度；最近 8 條、完整歷史 247 條`);
