...
async function run(html, label) {
  ...
  for (let attempt = 0; attempt < 2 && !ready; attempt++) {
    ...
    catch (e) { console.log(`   ⚠️ ${label} 第 ${attempt + 1} 次載入逾時${attempt ? '' : '，重試一次'}`); }
  }
  ...
  await p.evaluate(() => {
    ...
    setInterval(() => { const c = _trtcOfficialCorrect.count;
  ...
  const t = await p.evaluate(() => ({ toasts: window.__t.toasts, trig: window.__t.trig, byLine: window.__t.byLine,
    mis: window.__t.mis, good: window.__t.good,
  ...
  console.log(`  ${label.padEnd(20)} ${SEC}s 內跳 ${String(r.toasts).padStart(3)} 次通知`
    + `｜觸發累計 ${String(trig).padStart(5)} 台次｜間隔中位 ${r.medGap == null ? '—' : r.medGap.toFixed(1) + 's'}`
    + `｜名冊 ${t.veh} 台`);
  console.log(`     逐線觸發：${Object.entries(t.byLine).sort((a, b) => b[1] - a[1]).map(([k, v]) => `${k}:${v}`).join('  ') || '（無）'}`);
  console.log(`     歸因：官方真的重新報過 ${t.good} 次｜官方根本沒再看到那台車 ${t.mis} 次`);
  return r;
}
console.log(`官方訂正通知驗收：三組 × ${SEC}s，同一份正式站即時資料`);
const res = [];
for (const [label, html] of ARMS) res.push(await run(html, label));
...
console.log(`   修法組仍有 ${fix.trig} 次觸發＝官方真的訂正時照常跳（正向對照）`);
