 ...
 for(const width of [360,375,390,414,520,768,1280]){
  ...
  try{
   ...
   for(const theme of ['light','dark']){
    await page.evaluate(theme=>state._setAppearance(theme),theme);await page.waitForFunction(()=>!railIslandIntegration.loading&&railIslandIntegration.renderer?.stats.models>0);await page.waitForTimeout(350);
