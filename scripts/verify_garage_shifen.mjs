...
for(const [engine,type] of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  // 山的背面與夜燈（09-12 兩點：轉到背面山要是實心的、有房子有燈的地方要有光）。全部在 1440×1000 的全景頁上量真實像素；車先停到後直線，老街不被車擋。
  {
   ...
   check(engine+' 山鋪到台子後緣、後緣有一面封到地面的切面牆',facts.hillsY&&facts.hillsY[1]>=19.9&&facts.skirtY&&Math.abs(facts.skirtY[0]-20)<.05&&facts.skirtZ&&Math.abs(facts.skirtZ[0]-(G+.02))<.05&&facts.skirtZ[1]>G+.8,{hillsY:facts.hillsY,skirtY:facts.skirtY,skirtZ:facts.skirtZ});
