 ...
 try{
  ...
  // 夜燈與月台底下（09-12 使用者：「晚上路燈跟車站也都要有光」「高架車站下面看起來太空」）。在全景頁量真實像素；車先開到後直線，站區不被車擋。
  {
   ...
   const facts=await p.evaluate(async()=>{const S=await import('../../rail-3d/garage-scenes/viaduct.js'),T=await import('../../rail-3d/vendor/three.module.js'),s=S.createScene(),g=s.group,M=new T.Matrix4(),pos=new T.Vector3(),rot=new T.Quaternion(),scl=new T.Vector3();
    const inst=[];for(const o of g.children)if(o.isInstancedMesh)for(let i=0;i<o.count;i++){o.getMatrixAt(i,M);M.decompose(pos,rot,scl);inst.push({m:o.material.name||'',x:pos.x,y:pos.y,z:pos.z,sz:scl.z,sx:scl.x,sy:scl.y});}
   ...
   check(engine+' 夜裡站房與月台有光：站房玻璃暖亮、站體窗帶離入口燈遠仍自己發光、月台面對橋墩的亮度比晚上比白天高',night[2].l>day[2].l*1.25&&night[2].r>night[2].b&&night[6].l>day[6].l*1.25&&night[6].r>night[6].b+20&&platRatio(night)>platRatio(day)*1.2,{stationDay:day[2],stationNight:night[2],bandDay:day[6],bandNight:night[6],platDay:+platRatio(day).toFixed(2),platNight:+platRatio(night).toFixed(2)});
   check(engine+' 月台底下不是空的：站體整面牆從地面撐到月台底、橋下鋪面停車場有車有機車、正面看得到站體牆面不是草地',facts.hall>=1&&facts.hallWall>=1&&facts.paving>=1&&facts.cars>=5&&facts.scooters>=5&&day[5].l>120&&!(day[5].g>day[5].r+8&&day[5].g>day[5].b+20),{hall:facts.hall,hallWall:facts.hallWall,paving:facts.paving,cars:facts.cars,scooters:facts.scooters,hallPixel:day[5]});
  }
