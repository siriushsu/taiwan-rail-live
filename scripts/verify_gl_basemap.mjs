...
async function boot(browser, query, setup) {
...
try {
  ...
  // ── L2/L3 OFM 圖磚全掛 ──
  {
    const { ctx, page, errs } = await boot(browser, 'engine=maplibre', p => p.route('**/tiles.openfreemap.org/**', r => r.fulfill({ status: 500, body: '' })));
  ...
  // ── S 衛星(MapLibre):token/session 端點與 Esri 圖磚全部 route 攔截,不打真網路 ──
  {
    ...
    const { ctx, page, errs } = await boot(browser, 'engine=maplibre', async p => {
