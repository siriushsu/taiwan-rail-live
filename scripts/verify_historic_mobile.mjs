...
for(const [engine,type] of Object.entries({chromium,webkit})){
 ...
 for(const width of (process.env.WIDTHS?process.env.WIDTHS.split(',').map(Number):[360,375,414,768])){
  ...
  try{
   await p.goto((process.env.BASE_URL||'http://127.0.0.1:5232/')+'?scene=3d&g=all&map=landscape&lang=zh-TW&t=12:00');await p.waitForFunction(()=>state.ready&&window.railIslandIntegration?.renderer&&!window.railIslandIntegration?.loading,null,{timeout:60000});
   ...
   await p.waitForFunction(()=>window.railIslandIntegration?.renderer?.getStations().entries.find(e=>e.id==='presidential-office')?.ready,null,{timeout:30000});
