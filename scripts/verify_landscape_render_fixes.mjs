...
for(const [engine,type]of Object.entries({chromium,webkit})){
 ...
 await p.evaluate(async()=>{const {makePath}=await import('./rail-3d/integration/train-path.js'),route=railIslandPhysical.geometry.drawingWays('thsr_sched','#ef7821').find(r=>String(r.routeId)==='197206562'),path=makePath(route.coordinates);window.__linkouCenter=path.at(path.length/2).coordinate;window.__linkouFrame=railIslandIntegration.capture();window.__linkouRender=railIslandIntegration.render;railIslandIntegration.render=()=>{};window.__linkouUpdate=()=>railIslandIntegration.renderer.update({...__linkouFrame,routes:[route],vehicles:[],selectedVehicleId:null,followLock:false});railIslandSunlight.setEnabled(true);railIslandSunlight.update(true);M.raw.jumpTo({center:__linkouCenter,zoom:19.2,pitch:72,bearing:160});__linkouUpdate();});
