...
// 「官方名冊那批改動不准碰這幾個不相干的函式」的對照基準。預設是網站分支上
// 官方名冊工作開始前的那顆；在 App 分支上要改指向該分支併入前的 HEAD——
// App 分支自己早就改過 showFollowPanel(橫放版面重排)，拿網站基準比會把一個
// 與本批無關的既有差異報成「本批動到了」。改的是驅動條件(比誰)不是期望值
// (仍然要求逐字相同),突變測試照樣會轉紅。
const BASE_REF = process.env.TRTC_OFFICIAL_BASE_REF || '008d5d6';
const BASE_INDEX = execFileSync('git', ['show', `${BASE_REF}:index.html`], {
    ...
    try {
      for (const width of [360, 375, 414, 768]) {
        ...
        const frameDeadline = await page.evaluate(async () => {
          ...
          // 先空跑三個 rAF 讓合成器熱起來再訂 deadline：WebKit 首個 rAF 回呼常慢到
          // 120ms 以上，冷啟動時第一格就越過 deadline ⇒ before 恆 null、判準必假紅
          // (實測 375/414 兩欄間歇假紅,frames:1、after.exact 仍為 true)。
          // 這是取樣時序修正,不動判準:before.fraction<1 這道「不是本來就停在終點」的
          // 控制組原樣保留。
          await new Promise(resolve => {
            let warm = 0;
            const tick = () => (++warm >= 3 ? resolve() : requestAnimationFrame(tick));
            requestAnimationFrame(tick);
          });
          // 把同一筆官方車的 deadline 調到約 250ms 後，連續取每個真 rAF畫格。
          // before 必須尚未到站；第一個 now>=arrEpoch 的 frame 必須 bit-exact 在 to。
          const now = Date.now() / 1000, arrEpoch = now + .25;
          vehicle.run = .25; vehicle.arrEpoch = arrEpoch; delete vehicle.depEpoch;
