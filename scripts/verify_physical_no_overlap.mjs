...
//   B 兩車同時停在**同一個停車節點**：原本全日抽樣 188 筆。根因是派車的月台指派沒有把停站
//     佔用算進去（`dispatch.handoffs` 全表只有 1 筆而且是林鐵，所以不是折返接力）。
//     2026-09-12 用 `scripts/repair_physical_platforms.mjs` 局部換月台修掉一半以上：**188 → 51**。
//     剩下的 51 筆修不掉的原因有三種：該站在拓樸裡只有一個停車節點（太麻里）、換過去沒有可接的
//     進出路徑、換了反而更糟。再往下清要補拓樸或重解，不在這支與那支修復腳本的範圍。
//     🔴 重抓班表之後要重跑那支，否則新班次會回到同月台疊車（BASE_B 會當場紅給你看）。
