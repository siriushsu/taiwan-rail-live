...
// ══════════ T2:文案 gate(窄承諾 + 衛星用詞 + 準確度免費承諾 + 第1項不越界宣稱免費集合) ══════════
{
  ...
  // C 層抓的是**序列**不是單詞:否定標記 → 時間性副詞(再／又／另外／額外) → 收費類動詞。
  // 少了時間性副詞就只是在陳述現況(「網站不收費」),不是「此後永遠不收錢」的承諾。
  const NEG_PRICE_RE = /(不|未|無|沒|非).{0,4}(再|又|另外|額外).{0,4}(收費|收取|計費|加價|漲價|收錢|要錢)/;
  ...
  function bannedIn(str) {
    ...
    for (const clause of text.split(FINE_SEP)) {
      ...
      if (m) out.push({ w: m[0], why: '否定式價格承諾', ctx: ctxOf(text, fine + m.index, m[0]) });
  ...
  //     且「期限」不是承諾對象。(但「…也不會另外收費。」那一半 C 層咬得到。)
