  ...
  try{
   ...
   for(const id of (width===375?['391706267','860092299','103544834','966445954']:['391706267'])){
    ...
    for(const mode of ['flat','terrain']){
     if(await page.locator('.view-rail [data-view="map"]').isVisible())await (l=>l.tap())(page.locator('.view-rail [data-view="map"]'));else{await (l=>l.tap())(page.locator('#viewSettingsBtn'));await (l=>l.tap())(page.locator('.view-tabs [data-view="map"]'));}const control=page.locator('[data-rail3d="ground"] [data-value="'+mode+'"]');await control.scrollIntoViewIfNeeded();await control.tap();await page.locator('.view-close').tap();
     ...
     const detail=await page.evaluate(()=>{
      ...
      // v0912o 起平坦地圖讀柔化過的 flatOffsetM（docs/flat-bridge-grade-0912.md），offsetM 只留給結構／地下分類。
      const expectedRail=c=>{const level=__route.level(c.s);if(!terrain)return level.flatOffsetM??level.offsetM;return Number.isFinite(level.terrainHeightM)?level.terrainHeightM:ground(c)+level.offsetM;};
     ...
     // 平坦地圖沒有 DEM，lift 是顯示剖面本身；v0912o 以 200m 尺度柔化橋端引道，靠近洞口那一側會降到 1m 左右（966445954 實測 1.09），
     // 「整列離地 > 3m」只對地形成立。平坦改驗：不埋進地面，且整列至少一處仍明顯架高。
     const terrain=mode==='terrain';
     const pass=(bridgeCase?detail.structures.decks>0&&detail.structures.piers>0:detail.structures.piers===0&&detail.structures.decks===0)&&detail.poses.length===2&&detail.poses.every(p=>p.kind===(bridgeCase?'bridge':'surface')&&p.cars>0&&p.error<.01&&(bridgeCase?(terrain?p.lift[0]>3:p.lift[0]>-1&&p.lift[1]>3):p.lift[0]>-1&&p.lift[0]<2&&p.offset<.001))&&!detail.errors.length&&!detail.overflow;
