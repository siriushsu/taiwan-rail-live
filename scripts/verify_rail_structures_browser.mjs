...
for(const mode of ['flat','terrain']){await page.evaluate(mode=>{railIslandIntegration.renderer.setGroundMode(mode);__bridgeUpdate(true,__bridgeVehicles);},mode);await page.waitForFunction(()=>{__bridgeUpdate(true,__bridgeVehicles);return window.railIslandIntegration?.renderer?.stats.models===2;},null,{timeout:45000});const detail=await page.evaluate(()=>{const r=railIslandIntegration.renderer,terrain=r.stats.groundMode==='terrain',ground=c=>terrain?r.map.queryTerrainElevation(c.coordinate):0;
 // 期望軌面＝層位剖面的顯示高程：地形開時取建置端算好的絕對高程 terrainValues（09-12 露天段縱坡重算後全線都有，
 // 顯示剖面刻意不再逐點跟著 DEM），沒有的段落才退回「DEM＋層位」；地形關時就是層位。
 // lift>3 是獨立的正向對照：軌面離 DEM 至少 3 公尺，證明地形模式真的把這座高架放在地面之上
 // （露天求解器對高架的離地下限是 6 公尺，留 3 公尺給 DEM 取樣差），不然期望值與實作同源、恆真。
 const expectedRail=(c,route)=>{const level=route.level(c.s);if(!terrain)return level.offsetM;return Number.isFinite(level.terrainHeightM)?level.terrainHeightM:ground(c)+level.offsetM;};
 return {poses:r.stats.poseSamples.map(p=>{const route=__bridgeVehicles.find(v=>v.id===p.id).route;return {id:p.id,cars:p.cars.length,error:Math.max(...p.cars.map(c=>Math.abs(c.height-.65-expectedRail(c,route)))),lift:Math.min(...p.cars.map(c=>c.height-.65-ground(c)))};}),fallback:r.stats.modelFallbacks,errors:r.stats.errors};});rows.push({engine,width,mode,test:'雙向高鐵列車仍貼合同一軌面',pass:detail.poses.length===2&&detail.poses.every(p=>p.cars===12&&p.error<.00001&&p.lift>3)&&!detail.fallback.length&&!detail.errors.length,detail});}
...
if(width===1280){
 // 先解除 114 次的跟車再搬鏡頭：跟車鏡頭每一拍都把畫面拉回 114 次（z17.3、detail 1），與下面的 jumpTo 賽跑；
 // 09-12 實測 webkit 就是輸掉這場賽跑——close 那格永遠讀到 detail 1、頂點數是 114 次所在的畫面，不是鏡頭要量的地方。
 await page.evaluate(()=>clearFollow());await page.waitForTimeout(500);
