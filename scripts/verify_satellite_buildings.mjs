...
await Promise.all(Object.entries({chromium,webkit}).map(async ([name,engine])=>{
 ...
 for(const width of (process.env.WIDTHS||'1280,360,375,390,414,768').split(',').map(Number)){
  ...
  // v0914c 起 3D 與底圖設定搬進「觀看」面板的地圖分頁；「衛星」那一列改成地圖風格分段鈕（明／暗／衛星／地景）。
  const open=async()=>{const rail=page.locator('.view-rail [data-view="map"]');if(await rail.isVisible())await rail.tap();else{await page.locator('#viewSettingsBtn').tap();await page.locator('.view-tabs [data-view="map"]').tap();}};
  ...
  try{
   ...
   await open();await tap('#msBasemapSeg [data-map="sat"]');await page.locator('.view-close').tap();await ready();
   ...
   await page.locator('.view-close').tap();await page.evaluate(()=>new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r))));
   ...
   await tap('#msBasemapSeg [data-map="dark"]');await page.locator('.view-close').tap();await ready();
   ...
   await open();await tap('[data-rail3d="ground"] [data-value="terrain"]');await page.locator('.view-close').tap();await ready();
   ...
   await open();await tap('#map3dRow');await page.locator('.view-close').tap();
   check('建築開關可隱藏模型',await page.evaluate(()=>railIslandIntegration.renderer.getStations().entries.every(e=>!e.visible)));
   await open();await tap('#map3dRow');await page.locator('.view-close').tap();await ready();check('重開建築仍是原貌',(await material()).meshes.every(m=>m.opacity===1));
