...
for (const R of RB) {
  ...
  // 「讓位開始」必須是**只有一台在停站**的那一刻：讓的那台停、通過的那台在走。
  // 兩台都停在同一站是「對稱態」（各偏一半好讓兩塊牌不重疊），本來就不是讓位場景。
  // 只看位移大小的話，會在兩條緩動曲線的相位差上開窗——一台已爬到 8.4px、另一台還在 0.0，
  // 看起來像「一台退出、一台留線上」，其實只是動畫還沒追上（實測 1117/103 這一段 dpx 恆為 0＝
  // 兩台在同一個點，最後穩定在 12.1/12.1 的對稱態，卻被整段記成 61 次違規）。
  // 這也讓判準對時序毫釐之差敏感：多一次 boot fetch 就能讓它翻面，二分完全不單調。
  // 加上 dMe !== dYou 之後：修法前的樹 2e777ee~1 仍紅 669 次（牙更利），2e777ee 與現在皆綠，
  // 且三者「走到離開」都還是 22 段——分母沒縮水。
