...
for(const [name,engine]of Object.entries({chromium,webkit})){
 ...
 try{
  ...
  const boot=async query=>{await page.goto('http://127.0.0.1:5236/?lang=zh-TW&t=12:00'+query);await page.waitForFunction(()=>state.ready&&!!window.railIslandSunlight,null,{timeout:60000});await page.evaluate(()=>state.playing=false);};
  await boot('');
