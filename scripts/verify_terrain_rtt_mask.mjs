...
//  前置:地景底圖有 source-layer=building 的 fill 圖層、地形開著、兩次 F 之間 ≤NOISE px(否則是環境問題不是回歸)。
//  「S=F」一律指差 ≤NOISE px:無視窗真 GPU 同一狀態強迫重畫兩次實測差 0–3 px;卡舊的量級是 1100 px 以上。
//  L0 地景＋地形,遮罩第一次套用後 S=F。
//  L1／L2 工程遮罩開、關:building filter 真的換了(有 setFilter)、畫面真的變了(F 前後差 >2000 px),S=F。
//     只有「開」的方向看得到卡舊:關遮罩時立體建物回來,會把自己的地面輪廓整個蓋住,修前 L2 也在雜訊內,只當煙霧測試。
...
//  D0–D3 暗色＋地形:S=F(暗色沒有 2D 建物,修前也在雜訊內,確保修法沒弄壞)。
//  C  對照(證明 S/F 看得到卡舊):關遮罩後把 freeRtt 換成空函式再開遮罩,S 與 F 必須差 >5×NOISE px。
//     這塊遮罩範圍地面露出的 2D 輪廓約 1200 px(多數被立體建物擋住),修前的卡舊也是這個量。
// 用法:BASE_URL=http://127.0.0.1:5207/ node scripts/verify_terrain_rtt_mask.mjs
// 一律無視窗(2026-09-23 使用者裁示:測試不准開視窗搶焦點);channel:'chrome' 走真 GPU,預設 headless shell 是軟體算繪。
...
for(const dark of [false,true]){
  ...
  check(noise<=NOISE,`${name} 前置:兩次強迫重畫之間 ≤${NOISE} px(實測 ${noise};超過是環境雜訊,不是回歸)`);
  if(!dark)check(pre.fill.includes('building'),`${name} 前置:有 source-layer=building 的 2D fill 圖層(${pre.fill.join(',')||'無'})`);
  const s0=await diff(S0,F0);check(s0<=NOISE,`${dark?'D0':'L0'} ${name}＋地形 遮罩首次套用後 S=F(差 ${s0} px)`);
  ...
  check(s1<=NOISE,`${dark?'D1':'L1'} ${name}＋地形 遮罩開 S=F(差 ${s1} px)`);
  check(s2<=NOISE,`${dark?'D2':'L2'} ${name}＋地形 遮罩關 S=F(差 ${s2} px;煙霧測試)`);
  ...
  if(!dark){
    ...
    check(s4<=NOISE,`L4 ${name}＋地形 重載開始前先到一個圖磚事件,遮罩開 S=F(差 ${s4} px)`);
    await toggle(null);
    ...
    check(s5<=NOISE,`L5 ${name}＋地形 重載分幀回來,遮罩開 S=F(差 ${s5} px)`);
    ...
    check(c>5*NOISE,`C 對照:freeRtt 換成空函式後開遮罩,S 與 F 必須遠超過雜訊容許(${c} px > ${5*NOISE})`);
  }
