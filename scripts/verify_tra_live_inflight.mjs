...
//
// 這支量四件事(時間用可控的假時鐘,「被取消的 I/O」用永不 resolve 的 fetch 模擬):
//   T1 去重仍在:兩個 request 同時要刷新,TDX 只打一次(TDX 呼叫量不准增加)。
//   T2 超齡放掉:一發卡住的刷新滿上限之後,下一個 request 自己重刷並拿到新資料。
//   T3 有限等待:上限內搭便車的人最多等到那一發滿上限,就回舊值,而且自己不多打 TDX。
...
const { _la: api } = await import('../worker.js');
const MAX = api.TRA_LIVE_INFLIGHT_MAX_MS;
...
check('上限有導出給測試', Number.isFinite(MAX) && MAX > 0, `MAX=${MAX}`);
...
nowMs += MAX; apiMode = { delayMs: 1500 };
const f = within(call(), 5000);           // F:E 已超齡 ⇒ 放掉 E、自己重刷(1.5 秒)
