...
const lateSkipped=structuredClone(lateEnd);lateSkipped.stops[stop].stop=false;lateSkipped.stops[stop].depSec=lateSkipped.stops[stop].arrSec;assert.notEqual(bind(lateSkipped)?.basis,'retimed','鍵變了、停靠改通過不可沿用自己的股道');
const lateNames=structuredClone(lateEnd);[lateNames.stops[pass].name,lateNames.stops[pass+1].name]=[lateNames.stops[pass+1].name,lateNames.stops[pass].name];assert.notEqual(bind(lateNames)?.basis,'retimed','鍵變了、站序也變了不可沿用自己的股道');
...
const held=structuredClone(dispatch);held.plans[entry[0]].holds[pass].departure=30;assert.equal(createPlanBinding(held)(derived),null,'通過時間更新不可沿用舊待避');assert.equal(createPlanBinding(held)(changedStop),null,'改點不可沿用帶待避的舊計畫');assert.notEqual(createPlanBinding(held)(lateEnd)?.basis,'retimed','鍵變了也不可沿用帶待避的舊計畫');
