...
console.log('\n【H】標題文案');
const NAMES = { '2026-09-25': '中秋', '2026-10-09': '國慶', '2026-10-25': '光復節' };
...
// 🔴 這條專考 spanLabel【掃過整段找名稱】而不是只查 span.from。
// 光復節 10-25 是週日，連假從前一天的週六 10-24 起算，名稱落在第二天。
// H2 考不到這件事（中秋 09-25 剛好就是區間第一天）——把 spanLabel 換成 names[span.from]
// 的話 H1-H6 會【全部仍綠】，而真實的光復節會靜默退成「這個連假」。實測驗過。
chk('H3 名稱不在區間第一天也認得',
  spanLabel({ from: '2026-10-24', to: '2026-10-26', days: ['2026-10-24', '2026-10-25', '2026-10-26'] }, NAMES) === '光復節連假');
...
// 🔴 H7/H8 守住「短區間」那兩個分支。沒有它們的話,把最後一行整個換成 `return '本週末';`
// 測試會全綠——而真實資料下兩年有 14 天會被寫成不是事實的「本週末」。
chk('H7 平日的單日假期叫假日不叫本週末',
  spanLabel({ from: '2026-01-01', to: '2026-01-01', days: ['2026-01-01'] }, NAMES) === '假日');  // 週四的元旦
chk('H8 短區間有名稱就直接用名稱',
  spanLabel({ from: '2026-10-25', to: '2026-10-26', days: ['2026-10-25', '2026-10-26'] }, NAMES) === '光復節');  // 週日+週一補假
