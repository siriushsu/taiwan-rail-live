...
// 對同一種失敗形狀給兩套相反政策。方向安全——「公告留久一點」遠好過「假裝恢復」,這是本功能
// 唯一的鐵則——但**代價不是有界的,別誤讀成小事**(2026-07-28 修復輪 4 複審實測推翻了原本寫在
// 這裡的「代價有界」說法):
//   (a) degraded 是 **per-sys 不是 per-sub-source**,所以 News 進 catch 會連帶讓 TYMC **Alert
//       主通道**的公告也無法解除,不只新聞稿類。這個耦合是修復輪 4 才產生的——在那之前暖快取
//       失敗回 degraded:false,tymc 還解除得掉。
//   (b) 沒有時間上限。複審讓 News 連續失敗 72 小時,degraded 恆為 ["tymc"],連一則 UpdateTime
//       已 26 小時前(早該掉出 24 小時窗)的舊新聞稿都還在被送出與 upsert。不是「延後」,
//       News 不好起來就是無限期。
// 之所以仍然不加時間上限,理由只有一個:那要一併決定「超過上限後回空、還是帶過期旗標」的
// 產品面取捨,不在這輪範圍(現成答案可抄 metroAlertOpFallback:超過 30 分鐘就回 [])。
//
// TTL 命中的 fast path(下一行)仍然不算退化:它連 try 都沒進,不是失敗,是設計本身(News
// 更新慢,故意 10 分鐘才問一次上游)。若改成標退化,代價經複審兩路實測:約 82% 的 payload
// 世代會帶這個標記(算術 1-110/600=81.7%,用真生產節奏離散模擬 83.5%),而因為 degraded 是
// per-sys,那 82% 的時間連 TYMC **Alert 主通道**的公告也解除不掉(複審在鏡像上驗過:一則
// 真的消失的 Alert 公告從 cleared=1 變成 0)。所以不標。
//
// ⚠️ 但要知道 fast path 這條路自己有個沒補的洞:它回的是本 isolate ≤600 秒前的快照,而 D1 是
// 所有 isolate 的聯集——別的 isolate 剛寫進 D1 的新公告不在這份快照裡,就會被判解除。
// News 端點**完全健康也會發生**(fast path 根本不問上游,fetchImpl 呼叫 0 次),與失敗無關。
// 這不是 News 專屬:Alert op 的 110 秒 mem、traAlert(:152)、thsrAlert(:187) 是同一個結構,
